...
models_path = Path('src/lib/exam-models.ts')
models = models_path.read_text()
anchor = "const CMMG_EFFPO_COURSES = ['Enfermagem', 'Fisioterapia', 'Fonoaudiologia', 'Odontologia', 'Psicologia'];\n"
verified = """
...
"""
if 'UFMG_VERIFIED_COURSES' not in models:
    assert anchor in models
    models = models.replace(anchor, anchor + verified, 1)
models = models.replace("if (university === 'UFMG') return true;", "if (university === 'UFMG') return UFMG_VERIFIED_COURSES.has(course);")
models_path.write_text(models)
print('premium product proof layer and verified institution offerings integrated')
