import { buildRoadmap } from '../src/lib/admissions-roadmap.ts';
...
const questions=Array.from({length:24},(_,i)=>({
  ...
  area:i%2===0?'Matemática':'Natureza',
  skill_name:i%2===0?'Funções e gráficos':'Genética',
...
if(mathSubject&&chosenTopic){
  ...
  assert('declared topic-level difficulty reaches the weekly roadmap',manual.weeks.some(w=>w.topic===chosenTopic),chosenTopic);
  assert('roadmap explains that the declared difficulty influenced priority',manual.weeks.some(w=>w.topic===chosenTopic&&w.evidenceLabel.includes('dificuldade declarada')));
}
...
assert('photo/manual diagnostic reaches the weekly roadmap',photo.weeks.some(w=>w.topic==='Trigonometria'));
assert('diagnostic influence is visible in evidence label',photo.weeks.some(w=>w.topic==='Trigonometria'&&w.evidenceLabel.includes('diagnóstico por foto')));
