...
const checks=[
  ...
  ['catalog covers FGV without Link fallback',catalog.includes("examId==='fgv'")&&catalog.includes('FGV_PUBLIC_ADMIN')&&catalog.includes('FGV_ADMIN')],
  ['catalog covers Link explicitly',catalog.includes("examId==='link'")&&catalog.includes("label:'Jornada Link'")],
  ['catalog rejects unconfigured exam fallbacks',catalog.includes('const exhaustive:never=examId')],
  ...
  ['study twin combines declared and measured evidence',profiler.includes('buildStudyTwin')&&profiler.includes('student_practice_attempts')&&profiler.includes('student_skill_diagnostics')&&profiler.includes('conteúdo específico')],
  ...
  ['study twin uses progressive disclosure',profiler.includes('Escolher conteúdos')&&profiler.includes('started&&<>')],
  ['difficulty profiler supports topic search',profiler.includes('Busque: função quadrática, crase, MRUV, genética')],
  ['study twin requires topic-level difficulty',profiler.includes('A matéria serve só para abrir os conteúdos')&&profiler.includes('obrigatório para o gêmeo')&&profiler.includes('selectedDetails.length===0')],
  ['subject selection opens exact topics instead of assigning a whole subject',profiler.includes('openSubject')&&!profiler.includes('setSubjectLevel')&&!profiler.includes('Matéria inteira · nível')],
  ['declared difficulties persist subject and topic identity',profiler.includes('Dificuldade declarada: ${item.subject} > ${item.topic}')&&profiler.includes('subject:item.subject')&&profiler.includes('skill_name:item.topic')],
  ['top declared difficulties feed exact roadmap diagnostics',profiler.includes('selectedDetails.slice(0,12)')&&profiler.includes("evidence_path:'manual_difficulty'")&&profiler.includes("conectae:diagnostic-saved")],
  ...
  ['question bank separates every planner exam explicitly',['ENEM','CMMG','FUVEST','FGV','Insper','Ibmec','Einstein','Link'].every(x=>officialWorkspace.includes(`label:'${x}'`))],
  ['question bank fetch is scoped to the active exam',officialWorkspace.includes(".eq('exam_id',exam)")&&officialWorkspace.includes('.range(1000,1999)')],
  ...
  ['planner fetches questions for the active exam instead of a capped global pool',planner.includes(".eq('exam_id',model.examId)")&&planner.includes('.range(1000,1999)')],
  ['planner performance history is scoped to the active exam',planner.includes(".eq('exam_id',model.examId).order('created_at'")],
  ['guest answers feed the adaptive plan locally',planner.includes("GUEST_ATTEMPTS_KEY='conectae:planner-guest-attempts'")&&planner.includes('saveGuestAttempt')&&planner.includes('setAttempts(v=>[attempt,...v]')],
  ['tiny question samples have confidence-weighted influence',planner.includes('sampleConfidence=Math.min(1,sampleSize/8)')&&planner.includes('performanceMultiplier')],
  ['readiness combines declared scores with confidence-weighted measured performance',planner.includes("measuredProgress=Math.min(1,p.accuracy/.8)")&&planner.includes("measuredWeight=.35*p.sampleConfidence")],
