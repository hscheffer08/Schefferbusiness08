...
for(const marker of ['GOOGLE_GENERATIVE_AI_API_KEY','google(DIRECT_MODEL)','anonymousClientId','placeholder(url.hostname)','anon-${clientId}','sanitizeUnexpectedScript','script-repair','claude-fable-5'])if(!tutorPublic.includes(marker))throw new Error(`Tutor público sem resiliência obrigatória: ${marker}.`);
...
for(const marker of ['clientId: tutorClientId()','response.status !== 429','4000, 10000'])if(!tutorUi.includes(marker))throw new Error(`Interface da IA pública sem recuperação automática: ${marker}.`);
