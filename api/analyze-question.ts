...
const AI_FALLBACK_MODELS=['openai/gpt-5.4-mini','google/gemini-3.6-flash'];
...
export default async function handler(req:any,res:any){
  ...
  try{
    ...
    parsed.self_check_passed=Boolean(parsed.self_check_passed);
    parsed.assumptions=Array.isArray(parsed.assumptions)?parsed.assumptions.map(String).filter(Boolean).slice(0,3):[];
    if(parsed.needs_better_photo){parsed.confidence=Math.min(parsed.confidence,.55);parsed.correct_answer=null;}
