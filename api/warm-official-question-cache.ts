...
async function extractOne(req:any,q:Ref){
  const root=baseUrl(req);
  let d:any=null;
  if(q.series_id==='enem'&&q.year>=2019&&q.year<=2023){
    try{d=await getJson(`${root}/api/enem-official-questions?year=${q.year}&question=${q.question_number}`)}catch{}
  }
  if(!d?.found){
    if(!q.source_pdf_url)throw new Error('Sem PDF oficial');
    d=await getJson(`${root}/api/extract-official-question`,{
      method:'POST',headers:{'content-type':'application/json'},
      body:JSON.stringify({mode:'question',sourceUrl:q.source_pdf_url,questionNumber:q.question_number,exam:q.series_id.toUpperCase(),year:q.year,extractorVersion:VERSION})
    });
  }
...
async function pool<T,R>(items:T[],size:number,fn:(item:T)=>Promise<R>){
  ...
  await Promise.all(Array.from({length:Math.min(size,items.length)},async()=>{
    while(true){
      ...
      try{out[i]={ok:true,value:await fn(items[i])}}catch(e:any){out[i]={ok:false,error:String(e?.message||e)}}
    }
  }));
  return out;
}
...
export default async function handler(req:any,res:any){
  ...
  try{
    ...
    console.error('warm-official-question-cache failed',e?.message||e);
