...
export default async function handler(req:any,res:any){
  if(req.method==='GET')return json(res,200,{ok:true,service:'IA Conectaê',authMode:'vercel-ai-gateway-oidc',model:MODEL,verification:'single-pass-self-check'});
...
Faça a verificação dentro desta mesma resposta: reconstrua os dados uma segunda vez antes de finalizar. Não faça afirmações como "checado" sem realmente repetir a etapa decisiva.
...
No JSON final, evite LaTeX e NÃO use barras invertidas. Escreva fórmulas em texto simples/Unicode para manter o JSON válido.
...
Retorne APENAS JSON válido neste formato: {"answer":"resposta completa","educational":true,"resolved_doubt":true,"learning_focus":{"area":"área","skill_code":"código exato","skill_name":"nome exato","plan_skill_code":"código pai ou null","confidence":0.0,"reason":"motivo"},"offer_plan":true,"needs_better_image":false,"calculation_check":"resumo curto da segunda verificação feita na mesma solução"}`;
    ...
    try{parsed=cleanJson(raw)}catch(error){
      ...
      if(plain.length>40&&!/^\s*\{/.test(plain))parsed={answer:plain,educational:true,resolved_doubt:true,learning_focus:null,offer_plan:false,needs_better_image:false,calculation_check:'Resposta entregue em modo de recuperação.'};
      else throw error;
    }
    ...
    return json(res,200,{answer,educational:Boolean(parsed.educational),resolvedDoubt:Boolean(parsed.resolved_doubt),learningFocus:focus,offerPlan,needsBetterImage:Boolean(parsed.needs_better_image),referenceCoverage:allowedReference.length,model:MODEL,verified:true,verificationMode:'single-pass-self-check',calculationCheck:trim(parsed.calculation_check,1000)});
