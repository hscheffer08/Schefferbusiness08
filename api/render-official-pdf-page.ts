...
function norm(v:string){return v.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^A-Z0-9{} ]+/gi,' ').replace(/\s+/g,' ').trim().toUpperCase()}
function marker(text:string,n:number){const s=norm(text);return new RegExp(`\\bQUEST(?:AO|A0)\\s+0*${n}\\b`).test(s)||new RegExp(`(?:^|\\s)\\{?\\s*0*${n}\\s*\\}?\\s*(?:[.)-]|$)`).test(text)}
async function fetchPdf(sourceUrl:string){
 ...
 const r=await fetch(`${PROXY}?url=${encodeURIComponent(sourceUrl)}`,{headers:{...headers,...(SUPABASE_ANON_KEY?{apikey:SUPABASE_ANON_KEY,Authorization:`Bearer ${SUPABASE_ANON_KEY}`}:{})},signal:AbortSignal.timeout(25000)});
...
export default async function handler(req:any,res:any){
 ...
 try{
  ...
  if(!Number.isInteger(pageNumber)||pageNumber<1)pageNumber=await locate(pdf,questionNumber);
