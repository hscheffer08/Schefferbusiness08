        ...
        <section className="grid lg:grid-cols-[1.02fr_.98fr] gap-10 lg:gap-14 items-center pt-8 md:pt-14 lg:pt-20">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-9">
              {['50 cursos','1.000 opções','24 dimensões','Match por perfil'].map((item, index) => <span key={item} className={`rounded-full border px-3.5 py-2 text-xs font-bold ${index === 0 ? 'border-cyan-300/25 bg-cyan-300/10 text-cyan-200' : index === 1 ? 'border-fuchsia-300/20 bg-fuchsia-300/10 text-fuchsia-200' : index === 2 ? 'border-amber-300/20 bg-amber-300/10 text-amber-100' : 'border-white/10 bg-white/[0.04] text-ink-300'}`}>{item}</span>)}
            </div>
