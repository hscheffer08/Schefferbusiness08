...
function AccountControls() {
  ...
  return <div className="fixed top-4 right-5 md:right-10 z-[85] flex items-center gap-2">
    {user ? <>
      {isAdmin && <>
        <button type="button" onClick={() => setShowB2B(true)} className="hidden md:inline-flex items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-400/10 px-3.5 py-2 text-sm font-bold text-cyan-100 backdrop-blur-xl hover:bg-cyan-400/20 transition-colors"><Building2 className="w-4 h-4"/> B2B Insights</button>
        <button type="button" onClick={() => setShowAdmin(true)} className="hidden sm:inline-flex items-center gap-2 rounded-xl border border-violet-300/20 bg-violet-400/10 px-3.5 py-2 text-sm font-bold text-violet-100 backdrop-blur-xl hover:bg-violet-400/20 transition-colors"><Shield className="w-4 h-4"/> Painel</button>
      </>}
