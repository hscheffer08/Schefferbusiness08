...
import { ArrowLeft, ArrowRight, BookOpen, CheckCircle2, Compass, GraduationCap, Search, Sparkles, Trophy, ShieldCheck } from 'lucide-react';
import { ACADEMIC_AREAS, type AcademicArea } from '@/lib/area-match-data';
import { calculateProfessionalAreaMatches, professionalQuestionsForArea } from '@/lib/professional-area-matching';
...
export default function AreaMatchPortal({ onClose, initialAreaId }: Props) {
  ...
  const questions = area ? professionalQuestionsForArea(area) : [];
  const matches = useMemo(() => area ? calculateProfessionalAreaMatches(area, answers) : [], [area, answers]);
  ...
  return <div className="min-h-screen bg-ink-950 text-ink-50"><header className="px-5 md:px-10 py-5 flex items-center justify-between border-b border-ink-800"><button onClick={()=>setStep('areas')} className="inline-flex items-center gap-2 text-sm text-ink-300"><ArrowLeft className="w-4 h-4"/> Trocar área</button><div className="flex items-center gap-2 font-bold"><GraduationCap className="w-5 h-5 text-brand-400"/>Conecta<span className="text-brand-400">ê</span></div><button onClick={onClose} className="text-sm text-ink-400">Início</button></header><main className="max-w-5xl mx-auto px-5 py-12"><div className="mb-10"><span className="inline-flex items-center gap-2 text-brand-300 text-sm font-semibold mb-3"><Sparkles className="w-4 h-4"/> Fit personalizado · {area.name}</span><h1 className="text-4xl md:text-5xl font-bold mb-3">Onde você tende a se encaixar melhor</h1><p className="text-ink-400 max-w-3xl">O ranking agora separa aderência acadêmica, estilo de aprendizagem, ambiente, carreira e exposição global/impacto. Isso evita transformar fatores diferentes em uma nota opaca.</p></div><div className="space-y-4">{matches.map((m, idx) => <div key={m.university.id} className={`rounded-2xl border p-5 md:p-6 ${idx===0?'border-brand-400/60 bg-brand-500/10':'border-ink-800 bg-ink-900/60'}`}><div className="flex items-start gap-4"><div className={`w-11 h-11 rounded-xl flex items-center justify-center font-black ${idx===0?'bg-brand-500 text-ink-950':'bg-ink-800 text-ink-300'}`}>{idx+1}</div><div className="flex-1 min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="text-xl font-bold">{m.university.name}</h3>{idx===0 && <span className="inline-flex items-center gap-1 text-xs font-bold text-brand-300"><Trophy className="w-3.5 h-3.5"/> MELHOR FIT</span>}<span className="inline-flex items-center gap-1 text-[11px] text-ink-500"><ShieldCheck className="w-3.5 h-3.5"/> confiança dos dados {m.dataConfidence}%</span></div><p className="text-sm text-ink-500 mt-1">{m.university.course}</p><div className="grid grid-cols-2 md:grid-cols-5 gap-2 mt-4">{[['Acadêmico',m.academicFit],['Aprendizagem',m.learningFit],['Ambiente',m.environmentFit],['Carreira',m.careerFit],['Global/impacto',m.globalPurposeFit]].map(([label,score])=><div key={label as string} className="rounded-xl bg-ink-950/50 border border-ink-800 p-3"><div className="text-[11px] text-ink-500">{label}</div><div className="text-lg font-bold text-ink-200">{score}%</div></div>)}</div><div className="flex flex-wrap gap-2 mt-4">{m.topReasons.map(reason=><span key={reason} className="text-xs px-2 py-1 rounded-full bg-brand-500/10 text-brand-300">{reason}</span>)}</div></div><div className="text-right"><div className="text-3xl font-black text-brand-300">{m.fitScore}%</div><div className="text-[11px] text-ink-500">fit pessoal</div></div></div></div>)}</div><div className="mt-8 rounded-2xl border border-ink-800 bg-ink-900/50 p-5 text-sm text-ink-500 flex gap-3"><CheckCircle2 className="w-5 h-5 text-brand-400 flex-shrink-0"/><span><strong className="text-ink-300">Importante:</strong> fit pessoal não é ranking acadêmico nem chance de aprovação. A próxima camada do banco usa e-MEC/INEP, fontes oficiais do curso e dados de admissão separadamente, com nível de confiança por evidência.</span></div></main></div>;
}
