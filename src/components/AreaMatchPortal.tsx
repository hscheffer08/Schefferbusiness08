  ...
  if (step === 'areas') return <div className="min-h-screen bg-[#070b16] text-ink-50 relative">
    ...
    <main className="relative max-w-6xl mx-auto px-5 md:px-8 py-8 md:py-10 pb-24">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">{filtered.map((item,cardIndex)=><button key={item.id} onClick={()=>selectArea(item)} disabled={!dataReady} className={`group relative overflow-hidden text-left rounded-[22px] border disabled:opacity-50 disabled:cursor-wait bg-gradient-to-br ${CARD_TONES[cardIndex % CARD_TONES.length]} hover:-translate-y-1 hover:border-white/25 transition-all duration-300`}>
        <div className="h-32 overflow-hidden relative bg-[radial-gradient(circle_at_75%_20%,rgba(91,151,255,.32),transparent_45%),linear-gradient(135deg,#0a1b37,#0b1020)]"><img src={AREA_PHOTOS[cardIndex%AREA_PHOTOS.length]} alt="" loading="lazy" className="w-full h-full object-cover opacity-65 group-hover:scale-105 transition-all duration-500" referrerPolicy="no-referrer" onError={(event)=>{ event.currentTarget.style.display='none'; }}/><div className="absolute inset-0 bg-gradient-to-t from-[#0b1120] via-transparent to-transparent"/><span className="absolute top-3 right-3 text-[11px] px-2.5 py-1 rounded-full bg-black/35 border border-white/10">{item.universities.length} faculdades</span></div>
