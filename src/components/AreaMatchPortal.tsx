...
const CARD_TONES = [
  'from-amber-400/20 via-brand-500/10 to-transparent border-amber-300/15',
  'from-amber-400/15 via-amber-500/10 to-transparent border-amber-300/15',
  'from-amber-300/15 via-amber-500/10 to-transparent border-amber-300/15',
  'from-amber-300/15 via-amber-500/10 to-transparent border-amber-300/15',
];
...
export default function AreaMatchPortal({ onClose, initialAreaId }: Props) {
  ...
  if (step === 'areas') return <div className="min-h-screen bg-[#050505] text-ink-50 relative">
    <div className="fixed inset-0 pointer-events-none"><div className="absolute -left-36 top-20 w-[500px] h-[500px] rounded-full bg-amber-500/10 blur-[140px]"/><div className="absolute -right-40 top-[35%] w-[520px] h-[520px] rounded-full bg-amber-500/10 blur-[150px]"/></div>
    <header className="sticky top-0 z-20 border-b border-white/10 bg-[#050505]/95 backdrop-blur-2xl px-4 md:px-8 py-3.5">
      <div className="mx-auto max-w-6xl grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        ...
        <div className="flex items-center gap-2 font-extrabold"><div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-300 to-brand-500 flex items-center justify-center"><GraduationCap className="w-4 h-4 text-[#050505]"/></div><span>Conectaê</span></div>
        <div className="justify-self-end flex items-center gap-2">
          ...
          <button onClick={openPlanner} className="inline-flex items-center gap-2 rounded-xl border border-amber-300/20 bg-amber-300/[0.08] px-3 py-2 text-xs font-bold text-amber-100 hover:bg-amber-300/[0.14]"><Target className="w-4 h-4"/><span className="hidden sm:inline">Plano de aprovação</span><span className="sm:hidden">Plano</span></button>
        </div>
      </div>
    </header>
    <main className="relative max-w-6xl mx-auto px-5 md:px-8 py-8 md:py-10 pb-24">
      <section className="relative overflow-hidden rounded-[28px] border border-white/10 min-h-[300px] md:min-h-[320px] mb-7 shadow-2xl shadow-black/30 bg-[radial-gradient(circle_at_80%_15%,rgba(255,212,94,.24),transparent_40%),linear-gradient(135deg,#0b0904,#050505)]">
        <img src={HERO} alt="Campus universitário" className="absolute inset-0 w-full h-full object-cover" referrerPolicy="no-referrer" onError={(event)=>{ event.currentTarget.style.display='none'; }}/>
        <div className="absolute inset-0 bg-gradient-to-r from-[#050505] via-[#050505]/90 to-[#050505]/35"/>
        <div className="relative z-10 max-w-3xl p-7 md:p-10 min-h-[300px] md:min-h-[320px] flex flex-col justify-end">
          <span className="inline-flex w-fit items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1.5 text-amber-200 text-sm font-semibold mb-4"><Compass className="w-4 h-4"/> Já sabe seu curso?</span>
          <h1 className="text-4xl md:text-6xl font-black tracking-[-.035em] leading-[1.02] mb-4">Descubra onde você pode <span className="text-amber-200">se encaixar melhor.</span></h1>
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-5">
        <div><div className="text-xs font-bold uppercase tracking-[.14em] text-amber-200 mb-1">Todos os cursos</div><h2 className="text-2xl md:text-3xl font-black tracking-[-.03em]">Escolha entre {areas.length} áreas e cursos</h2><p className="text-sm text-ink-500 mt-1">Role a página para ver todos. Nenhum curso fica escondido atrás desta tela.</p></div>
        <div className="relative w-full md:max-w-md"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-500"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar área ou curso..." className="w-full rounded-2xl border border-white/10 bg-white/[0.045] py-4 pl-12 pr-4 outline-none focus:border-amber-300/40"/></div>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">{filtered.map((item,cardIndex)=><button key={item.id} onClick={()=>selectArea(item)} disabled={!dataReady} className={`group relative overflow-hidden text-left rounded-[22px] border disabled:opacity-50 disabled:cursor-wait bg-gradient-to-br ${CARD_TONES[cardIndex % CARD_TONES.length]} hover:-translate-y-1 hover:border-white/25 transition-all duration-300`}>
        <div className="h-32 overflow-hidden relative bg-[radial-gradient(circle_at_75%_20%,rgba(255,212,94,.32),transparent_45%),linear-gradient(135deg,#0b0904,#0b0904)]"><img src={AREA_PHOTOS[cardIndex%AREA_PHOTOS.length]} alt="" loading="lazy" className="w-full h-full object-cover opacity-65 group-hover:scale-105 transition-all duration-500" referrerPolicy="no-referrer" onError={(event)=>{ event.currentTarget.style.display='none'; }}/><div className="absolute inset-0 bg-gradient-to-t from-[#0b0904] via-transparent to-transparent"/><span className="absolute top-3 right-3 text-[11px] px-2.5 py-1 rounded-full bg-black/35 border border-white/10">{item.universities.length} faculdades</span></div>
        <div className="p-5"><div className="w-10 h-10 -mt-10 relative z-10 rounded-xl bg-[#0b0904] border border-white/10 text-amber-200 flex items-center justify-center mb-4"><BookOpen className="w-5 h-5"/></div><h3 className="font-black text-xl mb-1">{item.name}</h3><p className="text-sm text-amber-200 mb-2 font-medium">{item.courses}</p><p className="text-sm text-ink-500 leading-relaxed min-h-[60px]">{item.description}</p><span className="mt-5 inline-flex items-center gap-1.5 text-sm font-bold text-ink-300 group-hover:text-white">Fazer match <ArrowRight className="w-4 h-4"/></span></div>
      </button>)}</div>
  ...
  if (!dataReady) return <div className="min-h-screen bg-[#050505] text-ink-50 flex items-center justify-center px-6"><div className="text-center max-w-md"><div className="w-12 h-12 mx-auto mb-5 rounded-2xl border border-amber-300/20 bg-amber-300/10 flex items-center justify-center"><Database className="w-6 h-6 text-amber-200"/></div><h2 className="text-2xl font-black mb-2">Carregando banco profissional</h2><p className="text-sm text-ink-400">Estamos conectando perguntas, pesos e os 24 indicadores de perfil de cada faculdade antes de iniciar o match.</p></div></div>;
...
function ContinuousPercentInput({ value, low, high, onChange }: { value: number | undefined; low: string; high: string; onChange: (value:number)=>void }) {
  ...
  return <div className="mb-9">
    <div className="flex items-center justify-center gap-3 mb-5">
      <input aria-label="Percentual exato" type="number" min={0} max={100} step={0.1} value={value ?? ''} placeholder="50" onChange={e=>{ if(e.target.value==='') return; commit(Number(e.target.value)); }} className="w-32 rounded-2xl border border-amber-300/25 bg-amber-300/[0.07] px-4 py-3 text-center text-2xl font-black text-amber-100 outline-none focus:border-amber-300/60"/>
      <span className="text-2xl font-black text-amber-200">%</span>
    </div>
    <input aria-label="Escala contínua de 0 a 100" type="range" min={0} max={100} step={1} value={shown} onChange={e=>commit(Number(e.target.value))} className="w-full accent-amber-300"/>
