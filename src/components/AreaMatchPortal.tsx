...
export default function AreaMatchPortal({ onClose, initialAreaId }: Props) {
  ...
  const filtered = areas.filter(item => normalizeSearchText(`${item.name} ${item.courses}`).includes(normalizeSearchText(query)));
  ...
  if (step === 'areas') return <div className="min-h-screen bg-[#070b16] text-ink-50 relative">
    ...
    <header className="sticky top-0 z-20 border-b border-white/10 bg-[#070b16]/95 backdrop-blur-2xl px-4 md:px-8 py-3.5">
      <div className="mx-auto max-w-6xl grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <button aria-label="Voltar para descoberta" onClick={onClose} className="justify-self-start inline-flex items-center gap-2 text-sm text-ink-300 hover:text-white"><ArrowLeft className="w-4 h-4"/> <span className="hidden sm:inline">Início</span></button>
    ...
    <main className="relative max-w-6xl mx-auto px-5 md:px-8 py-8 md:py-10 pb-24">
      <section className="relative overflow-hidden rounded-[28px] border border-white/10 min-h-[300px] md:min-h-[320px] mb-7 shadow-2xl shadow-black/30 bg-[radial-gradient(circle_at_80%_15%,rgba(61,155,255,.24),transparent_40%),linear-gradient(135deg,#08152d,#050b17)]">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-5">
        ...
        <div className="relative w-full md:max-w-md"><Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-500"/><input aria-label="Buscar área ou curso" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar área ou curso..." className="w-full rounded-2xl border border-white/10 bg-white/[0.045] py-4 pl-12 pr-4 outline-none focus:border-cyan-300/40"/></div>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">{filtered.map((item,cardIndex)=><button key={item.id} onClick={()=>selectArea(item)} disabled={!dataReady} className={`group relative overflow-hidden text-left rounded-[22px] border disabled:opacity-50 disabled:cursor-wait bg-gradient-to-br ${CARD_TONES[cardIndex % CARD_TONES.length]} hover:-translate-y-1 hover:border-white/25 transition-all duration-300`}>
        <div className="h-32 overflow-hidden relative bg-[radial-gradient(circle_at_75%_20%,rgba(91,151,255,.32),transparent_45%),linear-gradient(135deg,#0a1b37,#0b1020)]"><img src={AREA_PHOTOS[cardIndex%AREA_PHOTOS.length]} alt="" loading="lazy" className="w-full h-full object-cover opacity-65 group-hover:scale-105 transition-all duration-500" referrerPolicy="no-referrer" onError={(event)=>{ event.currentTarget.style.display='none'; }}/><div className="absolute inset-0 bg-gradient-to-t from-[#0b1120] via-transparent to-transparent"/><span className="absolute top-3 right-3 text-[11px] px-2.5 py-1 rounded-full bg-black/35 border border-white/10">{item.universities.length} {item.universities.length === 1 ? 'faculdade' : 'faculdades'}</span></div>
  ...
  if (!area) return <div className="min-h-screen grid place-items-center p-6"><div><h1 className="text-2xl font-bold">Curso não encontrado</h1><button className="mt-4 rounded-xl bg-blue-600 px-5 py-3 text-white" onClick={()=>setStep('areas')}>Ver todos os cursos</button></div></div>;
