...
type Message={role:'user'|'assistant';content:string;confidenceLabel?:string;confidenceReason?:string;uncertaintyReason?:string|null;selfChecked?:boolean;webVerified?:boolean;sources?:Array<{title:string;url:string}>};
...
function normalizeHistory(value:unknown):Message[]{
  ...
  const clean=value.filter((m:any)=>m&&(m.role==='user'||m.role==='assistant')&&typeof m.content==='string'&&m.content.trim()).map((m:any)=>({role:m.role as 'user'|'assistant',content:m.content.trim(),confidenceLabel:typeof m.confidenceLabel==='string'?m.confidenceLabel:undefined,confidenceReason:typeof m.confidenceReason==='string'?m.confidenceReason:undefined,uncertaintyReason:typeof m.uncertaintyReason==='string'?m.uncertaintyReason:null,selfChecked:Boolean(m.selfChecked),webVerified:Boolean(m.webVerified),sources:Array.isArray(m.sources)?m.sources.filter((s:any)=>s&&typeof s.title==='string'&&/^https?:\/\//i.test(String(s.url||''))).slice(0,4):[]}));
...
export default function AIEducationTutor(){
  ...
  const refreshContext=async()=>{
    if(!supabase)return;const exam=localStorage.getItem('conectae:active-exam')||'enem';
    ...
    setIsPremium(premiumActive);if(premiumActive||String(user.app_metadata?.role||'').toLowerCase()==='admin')setRemainingQuestions(null);
  ...
  const send=async(textOverride?:string)=>{
    ...
    const recent=next.slice(-6);const requestMessages=recent.map((m,i)=>i===recent.length-1&&m.role==='user'?{...m,content:`${m.content}\n\n${INTERNAL_RESPONSE_DIRECTIVE}`} : m);
  ...
  const addFocusToPlan=async()=>{
    ...
    try{
      const{data:sessionData}=await supabase.auth.getSession();const user=sessionData.session?.user;if(!user)throw new Error('Faça login novamente.');const exam=studentContext.exam||'enem';const planCode=pendingFocus.plan_skill_code||pendingFocus.skill_code;
  ...
  return <>
    {open&&<div className={`fixed z-[120] flex flex-col overflow-hidden border border-[#234576] bg-[#041027] text-white shadow-2xl shadow-black/50 ${expanded?'inset-2 md:inset-6 rounded-[22px]':'bottom-20 right-2 w-[calc(100vw-16px)] max-w-[460px] h-[min(720px,calc(100vh-100px))] rounded-[22px] md:right-5'}`}>
      <div className="flex items-center gap-3 border-b border-[#173765] bg-[#06162f] px-4 py-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#246cff]"><Bot size={21}/></div><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5 text-sm font-extrabold">IA Conectaê {isAdmin?<span className="rounded-full bg-emerald-300/15 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-emerald-200">Admin ilimitado</span>:isPremium&&<span className="rounded-full bg-amber-300/15 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-amber-200">Premium</span>} <Sparkles size={14} className="text-[#72a5ff]"/></div><div className="truncate text-[11px] text-[#8ea8ca]">Tutor educacional · {studentContext.exam?.toUpperCase?.()||'sua prova'}{isAdmin||isPremium?' · IA sem limite diário':remainingQuestions!==null?` · ${remainingQuestions} perguntas restantes`:''}</div></div><button onClick={()=>setExpanded(v=>!v)} className="rounded-lg p-2 text-[#9fb5d4] hover:bg-[#102a54]" aria-label={expanded?'Reduzir':'Expandir'}>{expanded?<Minimize2 size={17}/>:<Maximize2 size={17}/>}</button><button onClick={closeTutor} className="rounded-lg p-2 text-[#9fb5d4] hover:bg-[#102a54]" aria-label="Fechar e reiniciar conversa"><X size={18}/></button></div>
      <div className="flex-1 overflow-y-auto px-3 py-4 md:px-4">
        ...
        <div className="space-y-3">{messages.map((m,i)=><div key={`${m.role}-${i}-${m.content.slice(0,16)}`} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}><div className={`max-w-[92%] whitespace-pre-wrap rounded-2xl px-3.5 py-3 text-sm leading-relaxed ${m.role==='user'?'bg-[#246cff] text-white':'border border-[#193a68] bg-[#071a38] text-[#dce9fa]'}`}>{m.content}{m.role==='assistant'&&m.confidenceLabel&&<div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-[#193a68] pt-2 text-[10px] font-bold"><span className={`rounded-full px-2 py-1 ${m.confidenceLabel==='Alta confiança'?'bg-emerald-400/10 text-emerald-200':m.confidenceLabel==='Confiança moderada'?'bg-amber-400/10 text-amber-200':'bg-rose-400/10 text-rose-200'}`}>{m.confidenceLabel}</span>{m.selfChecked&&<span className="rounded-full bg-[#12315f] px-2 py-1 text-[#a9c7ef]">Resposta revisada</span>}{m.webVerified&&<span className="rounded-full bg-[#12315f] px-2 py-1 text-[#a9c7ef]">Fonte verificada</span>}{m.confidenceReason&&<span className="w-full pt-1 font-medium leading-relaxed text-[#a9bddc]">Por quê: {m.confidenceReason}</span>}{m.uncertaintyReason&&<span className="w-full pt-1 font-medium leading-relaxed text-[#d9b7c1]">Limite: {m.uncertaintyReason}</span>}{m.sources&&m.sources.length>0&&<div className="w-full pt-1 font-medium text-[#a9c7ef]">Fontes: {m.sources.map((source,index)=><span key={source.url}>{index>0?' · ':''}<a href={source.url} target="_blank" rel="noreferrer" className="underline decoration-[#4d82cc] underline-offset-2 hover:text-white">{source.title}</a></span>)}</div>}</div>}</div></div>)}
