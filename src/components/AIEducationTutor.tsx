...
type LearningFocus={area:string;skill_code:string;skill_name:string;confidence:number;reason?:string};
...
export default function AIEducationTutor(){
  ...
  const addFocusToPlan=async()=>{
    ...
    try{
      ...
      const{data:existing}=await supabase.from('student_skill_diagnostics').select('id,created_at').eq('user_id',user.id).eq('exam_id',exam).eq('skill_code',pendingFocus.skill_code).order('created_at',{ascending:false}).limit(1);
