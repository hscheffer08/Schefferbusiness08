...
export default function AIEducationTutor(){
  ...
  useEffect(()=>{try{const raw=localStorage.getItem(historyKey);const normalized=normalizeHistory(raw?JSON.parse(raw):[]);setMessages(normalized);localStorage.setItem(historyKey,JSON.stringify(normalized))}catch{setMessages([])}},[historyKey]);
  useEffect(()=>{try{localStorage.setItem(historyKey,JSON.stringify(normalizeHistory(messages)))}catch{}},[messages,historyKey]);
  ...
  const send=async(textOverride?:string)=>{
    ...
    try{
      ...
      const response=await fetch('/api/education-tutor',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({messages:next.slice(-12),context:{...studentContext,...context},imageDataUrl:image||undefined})});
      ...
      // Falhas nunca entram no histórico: o aluno mantém texto e foto prontos para tentar novamente.
  ...
  const clearConversation=()=>{setMessages([]);setContext({});setImage('');setInput('');setError('');setPendingFocus(null);setPlanStatus('');try{localStorage.removeItem(historyKey)}catch{}};
    ...
    {open&&<div className={`fixed z-[120] flex flex-col overflow-hidden border border-[#234576] bg-[#041027] text-white shadow-2xl shadow-black/50 ${expanded?'inset-2 md:inset-6 rounded-[22px]':'bottom-20 right-2 w-[calc(100vw-16px)] max-w-[460px] h-[min(720px,calc(100vh-100px))] rounded-[22px] md:right-5'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 md:px-4">
        ...
        <div className="space-y-3">{messages.map((m,i)=><div key={`${m.role}-${i}-${m.content.slice(0,16)}`} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}><div className={`max-w-[92%] whitespace-pre-wrap rounded-2xl px-3.5 py-3 text-sm leading-relaxed ${m.role==='user'?'bg-[#246cff] text-white':'border border-[#193a68] bg-[#071a38] text-[#dce9fa]'}`}>{m.content}</div></div>)}
          {busy&&<div className="flex justify-start"><div className="inline-flex items-center gap-2 rounded-2xl border border-[#193a68] bg-[#071a38] px-3.5 py-3 text-sm text-[#9fb5d4]"><Loader2 size={16} className="animate-spin"/>Analisando conteúdo, habilidade e melhor explicação…</div></div>}
      ...
      {image&&<div className="border-t border-[#173765] bg-[#06162f] px-3 py-2"><div className="flex items-center gap-2"><img src={image} alt="Questão anexada" className="h-14 w-14 rounded-lg object-cover"/><div className="min-w-0 flex-1 text-xs text-[#a9bddc]">Questão pronta para análise. A foto só é descartada depois de uma resposta concluída.</div><button onClick={()=>setImage('')} className="rounded-lg p-2 text-[#9fb5d4] hover:bg-[#102a54]" aria-label="Remover imagem"><Trash2 size={16}/></button></div></div>}
      <div className="border-t border-[#173765] bg-[#06162f] p-3"><div className="flex items-end gap-2 rounded-2xl border border-[#234576] bg-[#031027] p-2 focus-within:border-[#4d82cc]"><label className="shrink-0 cursor-pointer rounded-xl p-2 text-[#8eb7ff] hover:bg-[#102a54]" title="Enviar uma questão"><ImagePlus size={19}/><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" hidden onChange={e=>chooseImage(e.target.files?.[0]||null)}/></label><textarea value={input} onChange={e=>{setInput(e.target.value);if(error)setError('')}} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}} rows={1} placeholder="Digite sua dúvida ou envie uma questão…" className="max-h-28 min-h-10 flex-1 resize-none bg-transparent px-1 py-2 text-sm text-white outline-none placeholder:text-[#607a9f]"/><button onClick={()=>send()} disabled={busy||!input.trim()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#246cff] disabled:opacity-40" aria-label="Enviar"><Send size={18}/></button></div><div className="mt-2 flex items-center justify-between gap-3 text-[10px] text-[#607a9f]"><span className="inline-flex items-center gap-1"><ShieldCheck size={11}/>Respostas contextualizadas pela sua prova; o plano só muda com confirmação.</span>{messages.length>0&&<button onClick={clearConversation} className="shrink-0 hover:text-white">Limpar conversa</button>}</div></div>
    </div>}
