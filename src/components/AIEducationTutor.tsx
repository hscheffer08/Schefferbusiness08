  ...
  return <>
    {open&&<div className={`fixed z-[120] flex flex-col overflow-hidden border border-[#234576] bg-[#041027] text-white shadow-2xl shadow-black/50 ${expanded?'inset-2 md:inset-6 rounded-[22px]':mobileDocked?'bottom-[calc(78px+env(safe-area-inset-bottom))] right-2 w-[calc(100vw-16px)] max-w-[460px] h-[min(720px,calc(100dvh-158px))] rounded-[22px] md:bottom-20 md:right-5 md:h-[min(720px,calc(100vh-100px))]':'bottom-20 right-2 w-[calc(100vw-16px)] max-w-[460px] h-[min(720px,calc(100vh-100px))] rounded-[22px] md:right-5'}`}>
      <div className="flex items-center gap-3 border-b border-[#173765] bg-[#06162f] px-4 py-3">
        ...
        <div className="min-w-0 flex-1"><div className="flex items-center gap-1.5 text-sm font-extrabold">IA Conectaê {isAdmin?<span className="rounded-full bg-emerald-300/15 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-emerald-200">Admin ilimitado</span>:isPremium&&<span className="rounded-full bg-amber-300/15 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-amber-200">Premium</span>} <Sparkles size={14} className="text-[#72a5ff]"/></div><div className="truncate text-[11px] text-[#8ea8ca]">Tutor educacional · {studentContext.exam?.toUpperCase?.()||'sua prova'}{isAdmin||isPremium?' · IA sem limite diário':remainingQuestions!==null?` · ${remainingQuestions} perguntas restantes`:''}</div></div>
        <button onClick={()=>setExpanded(v=>!v)} className="rounded-lg p-2 text-[#9fb5d4] hover:bg-[#102a54]" aria-label={expanded?'Reduzir':'Expandir'}>{expanded?<Minimize2 size={17}/>:<Maximize2 size={17}/>}</button>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 md:px-4">
        {!messages.length&&<div className="mx-auto max-w-sm py-6 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#0c2a5c] text-[#72a5ff]"><Sparkles size={27}/></div><h3 className="mt-4 text-xl font-extrabold">Tire a dúvida. Entenda o conteúdo. Evolua o plano.</h3><p className="mt-2 text-sm leading-relaxed text-[#9fb5d4]">{isPremium?'No Premium, a IA pode aprofundar explicações e planos sem limite diário de perguntas.':'Pergunte por texto ou anexe uma questão da Fototeca, câmera ou Arquivos. A IA confere a própria resposta antes de responder e mantém a explicação objetiva.'}</p><div className="mt-5 grid gap-2">{STARTERS.map(s=><button key={s} disabled={busy} onClick={()=>send(s)} className="rounded-xl border border-[#234576] bg-[#071a38] px-3 py-2.5 text-left text-xs font-bold text-[#c6d7ee] hover:border-[#3f72b8] hover:bg-[#0a2550] disabled:opacity-50">{s}</button>)}</div></div>}
        <div className="space-y-3">
          {messages.map((m,i)=><div key={`${m.role}-${i}-${m.content.slice(0,16)}`} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}>
            <div className={`max-w-[92%] whitespace-pre-wrap rounded-2xl px-3.5 py-3 text-sm leading-relaxed ${m.role==='user'?'bg-[#246cff] text-white':'border border-[#193a68] bg-[#071a38] text-[#dce9fa]'}`}>
              {m.content}
              {m.role==='assistant'&&m.confidenceLabel&&<div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-[#193a68] pt-2 text-[10px] font-bold"><span className={`rounded-full px-2 py-1 ${m.confidenceLabel==='Alta confiança'?'bg-emerald-400/10 text-emerald-200':m.confidenceLabel==='Confiança moderada'?'bg-amber-400/10 text-amber-200':'bg-rose-400/10 text-rose-200'}`}>{m.confidenceLabel}</span>{m.selfChecked&&<span className="rounded-full bg-[#12315f] px-2 py-1 text-[#a9c7ef]">Resposta revisada</span>}{m.webVerified&&<span className="rounded-full bg-[#12315f] px-2 py-1 text-[#a9c7ef]">Fonte verificada</span>}{m.confidenceReason&&<span className="w-full pt-1 font-medium leading-relaxed text-[#a9bddc]">Por quê: {m.confidenceReason}</span>}{m.uncertaintyReason&&<span className="w-full pt-1 font-medium leading-relaxed text-[#d9b7c1]">Limite: {m.uncertaintyReason}</span>}{m.sources&&m.sources.length>0&&<div className="w-full pt-1 font-medium text-[#a9c7ef]">Fontes: {m.sources.map((source,index)=><span key={source.url}>{index>0?' · ':''}<a href={source.url} target="_blank" rel="noreferrer" className="underline decoration-[#4d82cc] underline-offset-2 hover:text-white">{source.title}</a></span>)}</div>}</div>}
              {m.role==='assistant'&&<div className="mt-3 border-t border-[#193a68] pt-2 text-[10px] text-[#8ea8ca]">{m.feedback?<span className="font-semibold text-[#a9c7ef]">Obrigado — esse feedback melhora a avaliação da IA.</span>:<div className="flex items-center gap-2"><span>Resolveu sua dúvida?</span><button type="button" disabled={feedbackBusy!==null} onClick={()=>submitFeedback(i,true)} className="inline-flex items-center gap-1 rounded-lg border border-emerald-400/20 px-2 py-1 font-bold text-emerald-200 hover:bg-emerald-400/10 disabled:opacity-50"><ThumbsUp size={11}/>Sim</button><button type="button" disabled={feedbackBusy!==null} onClick={()=>submitFeedback(i,false)} className="inline-flex items-center gap-1 rounded-lg border border-rose-400/20 px-2 py-1 font-bold text-rose-200 hover:bg-rose-400/10 disabled:opacity-50"><ThumbsDown size={11}/>Não</button>{feedbackBusy===i&&<Loader2 size={11} className="animate-spin"/>}</div>}</div>}
            </div>
          </div>)}
      ...
      <div className="border-t border-[#173765] bg-[#06162f] p-3"><div className="flex items-end gap-2 rounded-2xl border border-[#234576] bg-[#031027] p-2 focus-within:border-[#4d82cc]"><label className="shrink-0 cursor-pointer rounded-xl p-2 text-[#8eb7ff] hover:bg-[#102a54]" title="Anexar da Fototeca, câmera ou Arquivos"><ImagePlus size={19}/><input type="file" accept="image/jpeg,image/png,image/webp" hidden onChange={e=>{chooseImage(e.target.files?.[0]||null);e.currentTarget.value='';}}/></label><textarea value={input} onChange={e=>{setInput(e.target.value);if(error)setError('')}} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}} rows={1} placeholder="Digite sua dúvida ou anexe uma questão…" className="max-h-28 min-h-10 flex-1 resize-none bg-transparent px-1 py-2 text-sm text-white outline-none placeholder:text-[#607a9f]"/><button onClick={()=>send()} disabled={busy||!input.trim()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#246cff] disabled:opacity-40" aria-label="Enviar"><Send size={18}/></button></div><div className="mt-2 flex items-center justify-between gap-3 text-[10px] text-[#607a9f]"><span className="inline-flex items-center gap-1"><ShieldCheck size={11}/>{isAdmin?'Administrador · uso ilimitado no produto.':isPremium?'Premium · IA sem limite diário de produto.':'Sessão renovada automaticamente · limite de 20 respostas por dia.'}</span>{messages.length>0&&<button onClick={clearConversation} className="shrink-0 hover:text-white">Limpar conversa</button>}</div></div>
    </div>}
