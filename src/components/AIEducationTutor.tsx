...
type Message={role:'user'|'assistant';content:string;confidenceLabel?:string;confidenceReason?:string;uncertaintyReason?:string|null;selfChecked?:boolean;webVerified?:boolean};
...
function normalizeHistory(value:unknown):Message[]{
  ...
  const clean=value.filter((m:any)=>m&&(m.role==='user'||m.role==='assistant')&&typeof m.content==='string'&&m.content.trim()).map((m:any)=>({role:m.role as 'user'|'assistant',content:m.content.trim(),confidenceLabel:typeof m.confidenceLabel==='string'?m.confidenceLabel:undefined,confidenceReason:typeof m.confidenceReason==='string'?m.confidenceReason:undefined,uncertaintyReason:typeof m.uncertaintyReason==='string'?m.uncertaintyReason:null,selfChecked:Boolean(m.selfChecked),webVerified:Boolean(m.webVerified)}));
...
async function tutorRequest(payload:unknown,premium=false){
  const endpoint=premium?'/api/education-tutor-premium':'/api/education-tutor';
  const call=async(token:string)=>fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify(payload)});
...
export default function AIEducationTutor(){
  ...
  const send=async(textOverride?:string)=>{
    ...
    try{
      const response=await tutorRequest(payload,isPremium);
      ...
      setMessages(normalizeHistory([...next,{role:'assistant',content:answer,confidenceLabel:data.confidenceLabel,confidenceReason:data.confidenceReason,uncertaintyReason:data.uncertaintyReason,selfChecked:data.selfChecked,webVerified:data.webVerified}]));
      setInput('');setImage('');setContext({});
      if(data.adminUnlimited)setIsAdmin(true);
      if(isPremium||data.adminUnlimited)setRemainingQuestions(null);else if(Number.isFinite(Number(data.remainingQuestions)))setRemainingQuestions(Number(data.remainingQuestions));
  ...
  return <>
    {open&&<div className={`fixed z-[120] flex flex-col overflow-hidden border border-[#234576] bg-[#041027] text-white shadow-2xl shadow-black/50 ${expanded?'inset-2 md:inset-6 rounded-[22px]':'bottom-20 right-2 w-[calc(100vw-16px)] max-w-[460px] h-[min(720px,calc(100vh-100px))] rounded-[22px] md:right-5'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 md:px-4">
        ...
        <div className="space-y-3">{messages.map((m,i)=><div key={`${m.role}-${i}-${m.content.slice(0,16)}`} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}><div className={`max-w-[92%] whitespace-pre-wrap rounded-2xl px-3.5 py-3 text-sm leading-relaxed ${m.role==='user'?'bg-[#246cff] text-white':'border border-[#193a68] bg-[#071a38] text-[#dce9fa]'}`}>{m.content}{m.role==='assistant'&&m.confidenceLabel&&<div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-[#193a68] pt-2 text-[10px] font-bold"><span className={`rounded-full px-2 py-1 ${m.confidenceLabel==='Alta confiança'?'bg-emerald-400/10 text-emerald-200':m.confidenceLabel==='Confiança moderada'?'bg-amber-400/10 text-amber-200':'bg-rose-400/10 text-rose-200'}`}>{m.confidenceLabel}</span>{m.selfChecked&&<span className="rounded-full bg-[#12315f] px-2 py-1 text-[#a9c7ef]">Resposta revisada</span>}{m.webVerified&&<span className="rounded-full bg-[#12315f] px-2 py-1 text-[#a9c7ef]">Fonte verificada</span>}{m.uncertaintyReason&&<span className="w-full pt-1 font-medium leading-relaxed text-[#d9b7c1]">Limite: {m.uncertaintyReason}</span>}</div>}</div></div>)}
