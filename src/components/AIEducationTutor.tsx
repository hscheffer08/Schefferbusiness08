...
export default function AIEducationTutor(){
  ...
  const[remainingQuestions,setRemainingQuestions]=useState<number|null>(null),[isPremium,setIsPremium]=useState(false);
  ...
  const refreshContext=async()=>{
    ...
    setIsPremium(premiumActive);if(premiumActive)setRemainingQuestions(null);
  ...
  const send=async(textOverride?:string)=>{
    ...
    try{
      ...
      if(isPremium)setRemainingQuestions(null);else if(Number.isFinite(Number(data.remainingQuestions)))setRemainingQuestions(Number(data.remainingQuestions));
  ...
  return <>
    {open&&<div className={`fixed z-[120] flex flex-col overflow-hidden border border-[#234576] bg-[#041027] text-white shadow-2xl shadow-black/50 ${expanded?'inset-2 md:inset-6 rounded-[22px]':'bottom-20 right-2 w-[calc(100vw-16px)] max-w-[460px] h-[min(720px,calc(100vh-100px))] rounded-[22px] md:right-5'}`}>
      <div className="flex items-center gap-3 border-b border-[#173765] bg-[#06162f] px-4 py-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#246cff]"><Bot size={21}/></div><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5 text-sm font-extrabold">IA Conectaê {isPremium&&<span className="rounded-full bg-amber-300/15 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-amber-200">Premium</span>} <Sparkles size={14} className="text-[#72a5ff]"/></div><div className="truncate text-[11px] text-[#8ea8ca]">Tutor educacional · {studentContext.exam?.toUpperCase?.()||'sua prova'}{isPremium?' · IA sem limite diário':remainingQuestions!==null?` · ${remainingQuestions} perguntas restantes`:''}</div></div><button onClick={()=>setExpanded(v=>!v)} className="rounded-lg p-2 text-[#9fb5d4] hover:bg-[#102a54]" aria-label={expanded?'Reduzir':'Expandir'}>{expanded?<Minimize2 size={17}/>:<Maximize2 size={17}/>}</button><button onClick={closeTutor} className="rounded-lg p-2 text-[#9fb5d4] hover:bg-[#102a54]" aria-label="Fechar e reiniciar conversa"><X size={18}/></button></div>
      ...
      <div className="border-t border-[#173765] bg-[#06162f] p-3"><div className="flex items-end gap-2 rounded-2xl border border-[#234576] bg-[#031027] p-2 focus-within:border-[#4d82cc]"><label className="shrink-0 cursor-pointer rounded-xl p-2 text-[#8eb7ff] hover:bg-[#102a54]" title="Anexar da Fototeca, câmera ou Arquivos"><ImagePlus size={19}/><input type="file" accept="image/jpeg,image/png,image/webp" hidden onChange={e=>{chooseImage(e.target.files?.[0]||null);e.currentTarget.value='';}}/></label><textarea value={input} onChange={e=>{setInput(e.target.value);if(error)setError('')}} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}} rows={1} placeholder="Digite sua dúvida ou anexe uma questão…" className="max-h-28 min-h-10 flex-1 resize-none bg-transparent px-1 py-2 text-sm text-white outline-none placeholder:text-[#607a9f]"/><button onClick={()=>send()} disabled={busy||!input.trim()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#246cff] disabled:opacity-40" aria-label="Enviar"><Send size={18}/></button></div><div className="mt-2 flex items-center justify-between gap-3 text-[10px] text-[#607a9f]"><span className="inline-flex items-center gap-1"><ShieldCheck size={11}/>{isPremium?'Premium · IA sem limite diário de produto.':'Sessão renovada automaticamente · limite de 20 respostas por dia.'}</span>{messages.length>0&&<button onClick={clearConversation} className="shrink-0 hover:text-white">Limpar conversa</button>}</div></div>
    </div>}
