...
export default function StudentStrategyCenter(){
  ...
  if(loading)return <section id="curso-estrategia" className="scroll-mt-20 bg-[#050505] px-4 py-10 text-white"><div className="mx-auto max-w-[1190px] rounded-[22px] border border-[#241904] bg-[#0b0904] p-6"><Loader2 className="animate-spin text-[#ffd45e]"/></div></section>;
  if(error)return <section id="curso-estrategia" className="scroll-mt-20 bg-[#050505] px-4 py-10 text-white"><div className="mx-auto max-w-[1190px] rounded-[22px] border border-[#241904] bg-[#0b0904] p-6"><TriangleAlert className="text-amber-300"/><p className="mt-3 text-sm text-[#e0aa18]">{error}</p></div></section>;

  const hasData=intelligence.valid.length>=3;
  const topError=intelligence.errorRanking[0];

  return <section id="curso-estrategia" className="scroll-mt-20 bg-[radial-gradient(circle_at_82%_0%,rgba(255,212,94,.14),transparent_30%),#050505] px-4 py-12 text-white">
    <div className="mx-auto max-w-[1190px]">
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div><div className="text-xs font-black uppercase tracking-[.17em] text-[#ffd45e]">Conectaê Intelligence</div><h2 className="mt-2 text-3xl font-black tracking-[-.04em] md:text-5xl">Seu sistema de decisão para passar.</h2><p className="mt-3 max-w-3xl text-sm leading-relaxed text-[#e0aa18]">O sistema transforma seu histórico em uma decisão prática: onde você está, o que mais derruba seu desempenho e qual ação tem maior retorno agora.</p></div>
        <div className="rounded-2xl border border-[#6f4f08] bg-[#171105] px-5 py-4"><div className="text-[10px] font-black uppercase tracking-[.14em] text-[#ffd45e]">Confiança dos dados</div><div className="mt-1 text-2xl font-black">{Math.round(intelligence.dataConfidence)}%</div><div className="text-[11px] text-[#b88408]">cresce conforme você treina</div></div>
      </div>

      {!hasData&&<div className="mb-6 rounded-[22px] border border-amber-300/20 bg-amber-300/[.05] p-5"><div className="flex items-start gap-3"><Sparkles className="mt-0.5 text-amber-200" size={20}/><div><strong>Seu gêmeo de aprendizagem ainda está sendo calibrado.</strong><p className="mt-1 text-sm text-[#e0aa18]">Faça pelo menos algumas questões e corrija um simulado. O painel deixa de usar um ponto neutro e passa a refletir seu comportamento real.</p><button onClick={()=>jump('correcao-simulado')} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-[#ffd45e] px-4 py-2.5 text-xs font-black">Fazer diagnóstico <ArrowRight size={14}/></button></div></div></div>}

      <div className="grid gap-4 lg:grid-cols-12">
        <article className="rounded-[24px] border border-[#6f4f08] bg-gradient-to-br from-[#171105] to-[#0b0904] p-6 lg:col-span-4"><div className="flex items-center justify-between"><div className="text-xs font-black uppercase tracking-[.14em] text-[#ffd45e]">Índice de prontidão</div><Gauge className="text-[#ffd45e]"/></div><div className="mt-5 flex items-end gap-2"><span className="text-6xl font-black tracking-[-.06em]">{intelligence.readiness??'—'}</span><span className="mb-2 text-sm font-bold text-[#b88408]">/100</span></div><p className="mt-3 text-sm text-[#e0aa18]">Não é “chance de aprovação”. É um índice interno de preparo baseado em acurácia, cobertura, constância, tendência e simulados.</p><div className="mt-5 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-black/15 p-3"><b>{intelligence.valid.length}</b><small className="block text-[#b88408]">questões</small></div><div className="rounded-xl bg-black/15 p-3"><b>{intelligence.uniqueSkills}</b><small className="block text-[#b88408]">habilidades</small></div><div className="rounded-xl bg-black/15 p-3"><b>{intelligence.activeDays}</b><small className="block text-[#b88408]">dias ativos</small></div></div></article>

        <article className="rounded-[24px] border border-amber-300/20 bg-amber-300/[.05] p-6 lg:col-span-8"><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-amber-200"><Crosshair size={16}/>Próximo melhor movimento</div><h3 className="mt-3 text-2xl font-black">{intelligence.weak?`Ataque ${intelligence.weak.label} agora.`:'Faça um diagnóstico curto antes de estudar no escuro.'}</h3>{intelligence.weak?<><p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#fffaf0]">Você acertou {Math.round(intelligence.weak.acc*100)}% em {intelligence.weak.attempts} tentativas nessa habilidade. O sistema priorizou esse ponto por combinar baixo desempenho com repetição suficiente para reduzir o risco de ser apenas acaso.</p><div className="mt-5 flex flex-wrap gap-2"><button onClick={()=>jump('curso-inicio')} className="inline-flex items-center gap-2 rounded-xl bg-amber-300 px-4 py-3 text-xs font-black text-[#050505]">Abrir treino direcionado <ArrowRight size={14}/></button><span className="rounded-xl border border-amber-300/20 px-4 py-3 text-xs font-bold text-amber-100">Missão sugerida: 8–12 questões</span></div></>:<p className="mt-2 text-sm text-[#fffaf0]">Sem histórico suficiente, qualquer prioridade seria chute. O sistema evita inventar uma fraqueza antes de ter dados.</p>}</article>

        <article className="rounded-[24px] border border-[#241904] bg-[#0b0904] p-6 lg:col-span-7"><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-[#ffd45e]"><Microscope size={16}/>Autópsia do erro</div><h3 className="mt-3 text-2xl font-black">Descubra por que você perde pontos — não só onde.</h3>{intelligence.wrong>0?<div className="mt-5 space-y-3">{intelligence.errorRanking.slice(0,5).map(e=><div key={e.key}><div className="mb-1 flex justify-between text-xs"><span className="font-bold text-[#ffd45e]">{e.label}</span><span className="text-[#b88408]">{e.count} · {Math.round(e.share)}%</span></div><div className="h-2 overflow-hidden rounded-full bg-[#241904]"><div className="h-full rounded-full bg-[#ffd45e]" style={{width:`${Math.max(4,e.share)}%`}}/></div></div>)}</div>:<p className="mt-3 text-sm text-[#e0aa18]">Quando você classificar os erros na correção de simulados, este painel separa conteúdo, interpretação, cálculo, distração e tempo.</p>}{intelligence.avoidable>0&&<div className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/[.05] p-4"><strong className="text-amber-100">Contrafactual:</strong><p className="mt-1 text-sm text-[#e0aa18]">{intelligence.avoidable} erros recentes foram classificados em categorias potencialmente recuperáveis sem aprender conteúdo novo. Se eles não tivessem ocorrido, a acurácia desse histórico iria de {Math.round(intelligence.accuracy)}% para cerca de {Math.round(intelligence.counterfactual)}%.</p></div>}</article>

        <article className="rounded-[24px] border border-[#241904] bg-[#0b0904] p-6 lg:col-span-5"><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-[#ffd45e]"><BrainCircuit size={16}/>Conectaê Twin</div><h3 className="mt-3 text-2xl font-black">Seu padrão, resumido.</h3><div className="mt-4 space-y-3 text-sm leading-relaxed text-[#e0aa18]"><p><b className="text-white">Desempenho:</b> {hasData?`${Math.round(intelligence.accuracy)}% de acerto no treino recente${intelligence.trend>2?', com tendência de melhora':intelligence.trend<-2?', com queda recente que merece atenção':', relativamente estável'}.`:'ainda sem amostra suficiente.'}</p><p><b className="text-white">Maior risco:</b> {intelligence.weak?`${intelligence.weak.label} (${Math.round(intelligence.weak.acc*100)}% de acerto).`:'ainda não identificado.'}</p><p><b className="text-white">Maior força:</b> {intelligence.strong?`${intelligence.strong.label} (${Math.round(intelligence.strong.acc*100)}% de acerto).`:'ainda não identificada.'}</p><p><b className="text-white">Ritmo:</b> {intelligence.medianSeconds?`mediana de ${Math.round(intelligence.medianSeconds/60*10)/10} min por questão registrada.`:'tempo por questão ainda sem dados suficientes.'}</p>{topError&&<p><b className="text-white">Erro dominante:</b> {topError.label}.</p>}</div></article>

        <article className="rounded-[24px] border border-[#241904] bg-[#0b0904] p-6 lg:col-span-12"><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-[#ffd45e]"><TrendingUp size={16}/>Simulador de futuro</div><div className="mt-3 grid gap-6 md:grid-cols-[1fr_1fr_220px] md:items-end"><label className="text-sm font-bold text-[#ffd45e]">Horas extras por semana: <b className="text-white">+{hoursBoost}h</b><input type="range" min="0" max="6" step="1" value={hoursBoost} onChange={e=>setHoursBoost(Number(e.target.value))} className="mt-3 w-full accent-[#ffd45e]"/></label><label className="text-sm font-bold text-[#ffd45e]">Redução de erros evitáveis: <b className="text-white">{errorReduction}%</b><input type="range" min="0" max="60" step="5" value={errorReduction} onChange={e=>setErrorReduction(Number(e.target.value))} className="mt-3 w-full accent-[#ffd45e]"/></label><div className="rounded-2xl border border-[#6f4f08] bg-[#171105] p-4 text-center"><div className="text-[10px] font-black uppercase text-[#ffd45e]">Prontidão simulada</div><div className="mt-1 text-4xl font-black">{projected??'—'}</div></div></div><p className="mt-4 text-xs leading-relaxed text-[#b88408]">Cenário exploratório, não previsão garantida. Serve para comparar decisões de estudo usando uma heurística conservadora sobre o seu próprio histórico.</p></article>
      </div>
    </div>
  </section>;
}
