...
export default function AdmissionsPlannerV11({onBack}:{onBack:()=>void}){
  ...
  const roadmap=useMemo(()=>buildRoadmap({model,course,priorities,weeklyHours:appliedWeeklyHours,questions:allowedQuestions}),[model,course,priorities,appliedWeeklyHours,allowedQuestions]);
  const relevantDiagnostics=useMemo(()=>diagnostics.filter(d=>d.exam_id===model.examId&&model.allowedQuestionAreas.some(a=>matchQuestionArea(d.area,a))).slice(0,5),[diagnostics,model]);
  ...
  return <div className="plan6">
    ...
    <main className="plan6-shell">
      ...
      {tab==='plano'&&<div className="plan6-grid">
        <section className="plan6-card span12"><div className="plan6-sectionlabel">Semana por semana</div><h2>{roadmap.weeks.length} semanas planejadas · {roadmap.dateLabel}</h2><p>Esse cronograma usa apenas as notas que você salvou. Novas dificuldades enviadas pelo scanner entram abaixo como <b>extras</b>, sem apagar a estrutura principal.</p><div className="plan6-strengths">{roadmap.milestones.map(m=><span className="plan6-chip active" key={`${m.date}-${m.label}`}>{fmtDate(m.date)} · {m.label}</span>)}</div></section>
        {relevantDiagnostics.length>0&&<section className="plan6-card span12"><div className="plan6-sectionlabel"><Sparkles size={14} style={{display:'inline',marginRight:6}}/>Extras adicionados pelas suas dificuldades</div><h2>O plano está atacando o que você enviou.</h2><p>Cada diagnóstico recente adiciona uma recuperação extra; ele não substitui a semana principal.</p>{relevantDiagnostics.map(d=>{const skill=d.diagnosis?.skill_name||d.skill_code||d.area;const video=`https://www.youtube.com/results?search_query=${encodeURIComponent(`${skill} ${model.examId} aula revisão`)}`;return <div className="plan6-statline" key={d.id}><div><div className="plan6-statname">EXTRA · {d.area} — {skill}</div><div className="plan6-statmeta">{recoveryAction(d.error_type,d.area,skill)}{d.error_detail?` • ${d.error_detail}`:''}</div><div className="plan6-actions" style={{marginTop:10}}><button className="plan6-btn primary" onClick={()=>openAreaQuestions(d.area)}><BookOpen size={14}/>Questões desta dificuldade</button><a className="plan6-btn" href={video} target="_blank" rel="noreferrer"><Video size={14}/>Vídeo de recuperação</a></div></div><div className="plan6-statvalue">extra</div></div>})}</section>}
        {roadmap.weeks.map(w=><section className="plan6-card span12" key={`${w.week}-${w.start}`}><div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}><div><div className="plan6-sectionlabel">Semana {w.week} · {fmtDate(w.start)}–{fmtDate(w.end)}</div><h2 style={{marginBottom:5}}>{w.focusLabel}</h2><p style={{marginBottom:0}}>{w.phase} · {w.topic}</p></div><div className="plan6-statvalue">~{w.hours}h foco</div></div><div className="plan6-grid" style={{marginTop:18}}><div className="plan6-callout blue span5"><strong><Video size={15} style={{display:'inline',marginRight:7}}/>Vídeo especializado</strong><p>{w.videoTitle}<br/><span style={{opacity:.75}}>{w.videoChannel}</span></p><a className="plan6-btn" href={w.videoUrl} target="_blank" rel="noreferrer"><PlayCircle size={14}/>Assistir aula do tema</a></div><div className="plan6-callout span4"><strong><BookOpen size={15} style={{display:'inline',marginRight:7}}/>Questões da semana</strong><p>Resolver <b>{w.questionTarget}</b> questões de {w.focusLabel}.</p><button className="plan6-btn primary" onClick={()=>openAreaQuestions(w.focusKey)}><BookOpen size={14}/>Abrir questões desta matéria</button></div><div className="plan6-callout span3"><strong>Checkpoint</strong><p>{w.checkpoint}</p></div></div></section>)}
      </div>}
