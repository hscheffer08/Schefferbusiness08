...
const RETAINED=new Set(['UFMG','USP','Faculdade Ciências Médicas de Minas Gerais','Insper','Link School of Business','Ibmec','Faculdade Israelita de Ciências da Saúde Albert Einstein']);
...
export default function AdmissionsPlannerV11({onBack}:{onBack:()=>void}){
  ...
  const miniSimulationSize=model.examId==='insper'?30:model.examId==='ibmec'||model.examId==='einstein'?25:20;
  const fullSimulationSize=model.examId==='insper'?60:model.examId==='ibmec'||model.examId==='einstein'?50:30;
  ...
  return <div className="plan6">
    ...
    <main className="plan6-shell">
      ...
      <section id="course-target-settings" className="plan6-selectors"><div id="course-target-course" className="plan6-field"><label>Curso</label><select value={selectedArea} onChange={e=>{setSelectedArea(e.target.value);setDirty(true)}}>{areas.map(a=><option key={a.area_id} value={a.area_id}>{a.courses||a.name}</option>)}</select></div><div id="course-target-university" className="plan6-field"><label>Faculdade</label><select value={selectedUniversity} onChange={e=>{setSelectedUniversity(e.target.value);setDirty(true)}}>{filteredUniversities.map(u=><option key={u.area_university_id} value={u.area_university_id}>{u.university_name}</option>)}</select></div><button className="plan6-save" disabled={saving} onClick={save}>{saving?<Loader2 size={16} className="animate-spin"/>:<Save size={16}/>}Salvar curso, faculdade e atualizar plano</button></section>
