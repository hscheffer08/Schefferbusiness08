import { ArrowRight, BarChart3, BookOpenCheck, Brain, Camera, CheckCircle2, Compass, FileText, GraduationCap, MessageSquareText, Sparkles, Target, Trophy } from 'lucide-react';
...
const features = [
  ...
  { icon: BarChart3, title: 'Correção de simulados', text: 'ENEM 2024 e 2025 com gabarito automático, além de CMMG, Seriado UFMG e diagnóstico por erro.' },
  { icon: FileText, title: 'Redação e discursivas', text: 'Prática de redação e questões abertas integrada ao mesmo plano para a preparação não ficar fragmentada.' },
  ...
  { icon: Trophy, title: 'Meta por vestibular', text: 'A estrutura muda conforme ENEM, FUVEST, Insper, Link, Ciências Médicas-MG ou Seriado UFMG.' },
];
...
export default function CourseHome() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#020817] text-white font-['Plus_Jakarta_Sans']">
      ...
      <header className="relative z-10 border-b border-white/5 bg-[#020817]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 md:px-8">
          <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#246cff] shadow-lg shadow-[#246cff]/25">
              <GraduationCap className="h-5 w-5" />
            </span>
            <div>
              <div className="text-lg font-black tracking-tight">Conecta<span className="text-[#72a5ff]">ê</span></div>
              <div className="text-[10px] font-bold uppercase tracking-[.18em] text-[#7891b4]">Curso de aprovação</div>
            </div>
          </div>
          <nav className="hidden items-center gap-5 text-sm font-bold text-[#9fb5d4] lg:flex">
            ...
            <button onClick={openInterview} className="transition hover:text-white">Entrevistas</button>
      ...
      <div className="relative z-10 border-b border-white/5 bg-[#041027] px-5 py-2 lg:hidden">
        <div className="mx-auto flex max-w-xl items-center justify-center">
          <button onClick={openInterview} className="flex items-center gap-2 text-xs font-extrabold text-[#a9c7ef] sm:text-sm">
            <MessageSquareText className="h-4 w-4" />Treino de entrevista
          </button>
        </div>
      </div>
      ...
      <main className="relative z-10">
        <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-16 pt-14 md:grid-cols-[1.1fr_.9fr] md:px-8 md:pb-24 md:pt-20">
          <div className="flex flex-col justify-center">
            <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-300/[.07] px-3 py-1.5 text-xs font-extrabold text-emerald-200">
        ...
        <section className="mx-auto max-w-7xl px-5 py-14 md:px-8 md:py-20">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <button onClick={openCourse} className="group rounded-[26px] border border-[#31588e] bg-gradient-to-br from-[#0b2856] to-[#06152f] p-6 text-left transition hover:-translate-y-1 hover:border-[#72a5ff] md:p-7"><Target className="h-7 w-7 text-[#72a5ff]" /><div className="mt-6 text-xs font-black uppercase tracking-[.15em] text-[#72a5ff]">Principal</div><h3 className="mt-2 text-2xl font-black tracking-tight">Curso de Aprovação</h3><p className="mt-3 text-sm leading-relaxed text-[#a9bddc]">Para quem já tem um objetivo e quer transformar esse objetivo em execução diária.</p><span className="mt-6 inline-flex items-center gap-2 font-black">Entrar no Curso <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span></button>
            <button onClick={openOfficialBank} className="group rounded-[26px] border border-emerald-400/25 bg-emerald-400/[.06] p-6 text-left transition hover:-translate-y-1 hover:border-emerald-300/55 md:p-7"><BookOpenCheck className="h-7 w-7 text-emerald-300" /><div className="mt-6 text-xs font-black uppercase tracking-[.15em] text-emerald-300">Banco especial</div><h3 className="mt-2 text-2xl font-black tracking-tight">Questões dos Vestibulares</h3><p className="mt-3 text-sm leading-relaxed text-[#a9bddc]">1.406 questões oficiais interativas, identificadas por vestibular, edição, número e área, com prova e gabarito rastreados.</p><span className="mt-6 inline-flex items-center gap-2 font-black text-emerald-200">Abrir banco oficial <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span></button>
            <button onClick={openCollegeTools} className="group rounded-[26px] border border-[#173765] bg-[#051127] p-6 text-left transition hover:-translate-y-1 hover:border-[#31588e] md:p-7"><Compass className="h-7 w-7 text-cyan-300" /><div className="mt-6 text-xs font-black uppercase tracking-[.15em] text-[#839ab9]">Complementar</div><h3 className="mt-2 text-2xl font-black tracking-tight">Explorar cursos e faculdades</h3><p className="mt-3 text-sm leading-relaxed text-[#9fb5d4]">Teste vocacional, match de faculdades, comparação e perfil continuam disponíveis para quem ainda está decidindo.</p><span className="mt-6 inline-flex items-center gap-2 font-black text-cyan-200">Explorar opções <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span></button>
          </div>
        </section>
      </main>
    </div>
