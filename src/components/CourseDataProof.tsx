...
export default function CourseDataProof({ compact = false }: { compact?: boolean }) {
  ...
  if (compact) {
    return (
      <div className="mx-auto max-w-[1240px] px-3 pt-3 md:px-6">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl border border-[#241904] bg-[#0b0904]/85 px-4 py-3 text-[11px] font-bold text-[#e0aa18] shadow-lg shadow-black/10">
          <span className="inline-flex items-center gap-1.5 text-amber-200"><ShieldCheck size={14}/>Base de inteligência verificada</span>
          ...
          <span className="ml-auto text-[#b88408]">{live ? 'dados carregados da base agora' : 'último snapshot auditado'}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <section className="border-y border-white/5 bg-[#0b0904]">
      <div className="mx-auto max-w-7xl px-5 py-14 md:px-8 md:py-20">
        <div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/[.07] px-3 py-1.5 text-xs font-extrabold text-amber-200"><ShieldCheck className="h-4 w-4"/>BASE AUDITADA</div>
            <h2 className="mt-5 text-3xl font-black tracking-[-.045em] md:text-5xl">Profundidade que aparece nos dados, não só no design.</h2>
            <p className="mt-4 max-w-xl text-sm leading-7 text-[#e0aa18] md:text-base">O Curso cruza estrutura de prova, referências oficiais, desempenho do aluno, banco de questões e histórico de erros. Quando um dado não é confiável, o produto mostra incerteza em vez de inventar precisão.</p>
            <div className="mt-5 inline-flex items-center gap-2 text-xs font-bold text-[#b88408]"><Database className="h-4 w-4"/>{live ? 'Contagens carregadas da base em tempo real.' : 'Exibindo o último snapshot técnico auditado.'}</div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {cards.map(({ icon: Icon, value, label, detail }) => (
              <article key={label} className="group rounded-[22px] border border-[#241904] bg-gradient-to-b from-[#0b0904] to-[#0b0904] p-5 shadow-xl shadow-black/15 transition duration-300 hover:-translate-y-1 hover:border-[#6f4f08]">
                <div className="flex items-start justify-between gap-4"><div><div className="text-3xl font-black tracking-[-.04em] text-white">{value}</div><div className="mt-1 text-xs font-extrabold uppercase tracking-[.09em] text-[#ffd45e]">{label}</div></div><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#ffd45e]/12 text-[#ffd45e]"><Icon className="h-5 w-5"/></span></div>
                <p className="mt-4 text-xs leading-6 text-[#e0aa18]">{detail}</p>
              </article>
            ))}
          </div>
        </div>
