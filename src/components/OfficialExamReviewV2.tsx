...
export default function OfficialExamReviewV2() {
  ...
  const [bookletKeyCoverage, setBookletKeyCoverage] = useState<number | null>(null);
  ...
  const booklets = useMemo(() => allBooklets.filter((item) => item.edition_id === editionId), [allBooklets, editionId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!supabase) return;
      const [{ data: editionData }, { data: bookletData }] = await Promise.all([
        supabase.from('official_exam_editions').select('id,year,application,application_label').eq('series_id', 'enem').order('year', { ascending: false }),
        supabase.from('official_exam_booklets').select('id,edition_id,day,booklet_code,color,question_start,question_end,source_pdf_url,answer_key_url').order('day').order('booklet_code'),
      ]);
      if (!active) return;
      const bookletRows = (bookletData ?? []) as Booklet[];
      const usableEditionIds = new Set(bookletRows.map((item) => item.edition_id));
      const rows = ((editionData ?? []) as Edition[]).filter((edition) => usableEditionIds.has(edition.id));
      setAllBooklets(bookletRows);
      setEditions(rows);
      const first = rows[0];
  ...
  useEffect(() => {
    if (!isEnem || !editionId) { setBookletId(''); return; }
    setBookletId(booklets[0]?.id ?? '');
  }, [booklets, editionId, isEnem]);

  useEffect(() => {
    let active = true;
    (async () => {
      if (!supabase || !isEnem || !bookletId) { setBookletKeyCoverage(null); setCheckingKey(false); return; }
      setCheckingKey(true);
      const { count, error } = await supabase.from('official_exam_item_booklet_map')
        .select('id', { count: 'exact', head: true }).eq('booklet_id', bookletId);
      if (!active) return;
      setBookletKeyCoverage(error ? 0 : count ?? 0);
  ...
  async function loadMappings() {
    ...
    const manualKey = parseAnswerSheet(officialKeyRaw, start, end);
    const manualMappings = [...manualKey.entries()].map(([number, answer]) => ({
    ...
    if (!isEnem) return manualMappings;
    if (!bookletId) return manualMappings;
    ...
    manualMappings.forEach((row) => {
      const existing = selected.get(row.question_number);
      if (!existing?.correct_option) selected.set(row.question_number, existing ? { ...existing, correct_option: row.correct_option, answer_status: row.answer_status } : row);
    });
  ...
  const saveDiagnosis = async () => {
    ...
    try {
      ...
      const scoresToSave = mode === 'answers' ? analysis.areaScores : [];
      ...
      setMessage(mode === 'answers'
        ? `Resultado salvo: ${mistakes.length} erro(s) e ${scoresToSave.length} desempenho(s) por área enviados ao seu Plano.`
        : `Diagnóstico salvo: ${mistakes.length} erro(s) enviados ao seu Plano. Suas notas não foram alteradas porque você informou somente as questões erradas.`);
  ...
  return <section id="correcao-simulado" className="max-w-[1180px] mx-auto px-4 md:px-6 py-8 text-white font-['Plus_Jakarta_Sans'] scroll-mt-24">
    <div className="rounded-[24px] border border-[#241904] bg-[#0b0904] p-5 md:p-7">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><div className="flex items-center gap-2 text-xs font-extrabold text-[#ffd45e]"><BookOpenCheck size={16}/>CORREÇÃO COMPLETA</div><h2 className="mt-2 text-2xl md:text-4xl font-extrabold tracking-[-.04em]">Corrija ENEM ou CMMG de verdade.</h2><p className="mt-2 max-w-3xl text-sm text-[#e0aa18]">Compare respostas com o gabarito, veja acertos por matéria, classifique cada erro e envie uma recuperação precisa para o seu Plano.</p></div>
        <div className="rounded-2xl border border-[#241904] bg-[#0b0904] px-4 py-3"><div className="text-2xl font-extrabold text-[#ffd45e]">{practice.length}</div><div className="text-[11px] font-bold text-[#e0aa18]">QUESTÕES DE TREINO DISPONÍVEIS</div></div>
      </div>

      <div className="mt-6 grid gap-3 md:grid-cols-3">
        <label className="text-xs font-bold text-[#e0aa18]">Prova<select value={exam} onChange={(event) => setExam(event.target.value as ExamChoice)} className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white"><option value="enem">ENEM oficial</option><option value="cmmg-medicina">CMMG — Medicina (60 objetivas)</option><option value="cmmg-effpo">CMMG — EFFPO (40 objetivas)</option></select></label>
        {isEnem ? <><label className="text-xs font-bold text-[#e0aa18]">Ano<select value={year} onChange={(event) => setYear(Number(event.target.value))} className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white">{availableYears.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-xs font-bold text-[#e0aa18]">Aplicação<select value={editionId} onChange={(event) => setEditionId(event.target.value)} className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white">{availableEditions.map((edition) => <option key={edition.id} value={edition.id}>{edition.application_label || edition.application}</option>)}</select></label></> : <label className="text-xs font-bold text-[#e0aa18]">Ano<input type="number" value={year} onChange={(event) => setYear(Number(event.target.value))} className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white"/></label>}
      </div>
      {isEnem && <div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-xs font-bold text-[#e0aa18]">Dia e caderno<select value={bookletId} onChange={(event) => setBookletId(event.target.value)} className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white">{booklets.length ? booklets.map((item) => <option key={item.id} value={item.id}>Dia {item.day} · {item.color} · caderno {item.booklet_code}</option>) : <option value="">Caderno ainda não indexado</option>}</select></label><label className="text-xs font-bold text-[#e0aa18]">Língua estrangeira<select value={language} onChange={(event) => setLanguage(event.target.value)} className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white"><option value="ingles">Inglês</option><option value="espanhol">Espanhol</option></select></label></div>}

      {isEnem && bookletId && <div className={`mt-3 rounded-xl border p-3 text-sm ${!needsManualKey ? 'border-amber-300/25 bg-amber-300/[.06] text-amber-100' : 'border-amber-300/25 bg-amber-300/[.06] text-amber-100'}`}>
      ...
      <div className="mt-6 flex flex-wrap gap-2">{([['answers', 'Tenho minhas respostas'], ['errors', 'Só sei quais errei']] as [ReviewMode, string][]).map(([value, label]) => <button key={value} onClick={() => setMode(value)} className={`rounded-xl border px-4 py-2.5 text-sm font-extrabold ${mode === value ? 'border-[#ffd45e] bg-[#ffd45e] text-white' : 'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>{label}</button>)}</div>

      {mode === 'answers' ? <div className={`mt-4 grid gap-4 ${needsManualKey ? 'lg:grid-cols-2' : ''}`}>
        {needsManualKey && <label className="text-xs font-bold text-[#e0aa18]">Gabarito oficial<textarea value={officialKeyRaw} onChange={(event) => setOfficialKeyRaw(event.target.value)} className="mt-2 min-h-[128px] w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white" placeholder="Cole a sequência oficial completa: ABCDE... ou 1-A, 2-C, 3-B... Use - para anulada."/><span className={`mt-1 block font-normal ${manualKeyComplete ? 'text-amber-300' : 'text-[#b88408]'}`}>{recognizedKey}/{expectedQuestions} respostas do gabarito reconhecidas</span></label>}
        <label className="text-xs font-bold text-[#e0aa18]">Suas respostas<textarea value={answerRaw} onChange={(event) => setAnswerRaw(event.target.value)} className="mt-2 min-h-[128px] w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white" placeholder="Cole a sequência completa. Use - para questão deixada em branco."/><span className={`mt-1 block font-normal ${answersComplete ? 'text-amber-300' : 'text-[#b88408]'}`}>{recognizedAnswers}/{expectedQuestions} respostas reconhecidas · questões {start}–{end}</span></label>
      </div> : <label className="mt-4 block text-xs font-bold text-[#e0aa18]">Questões erradas<textarea value={wrongRaw} onChange={(event) => setWrongRaw(event.target.value)} className="mt-2 min-h-[100px] w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white" placeholder="Ex.: 12, 37, 84, 102 ou 90-95"/><span className="mt-1 block font-normal text-[#b88408]">{wrongNumbers.length} número(s) reconhecido(s)</span></label>}

      <div className="mt-4 flex flex-wrap gap-2"><button onClick={runAnalysis} disabled={busy || checkingKey || (isEnem && !bookletId) || (mode === 'answers' && (!answersComplete || (needsManualKey && !manualKeyComplete)))} className="inline-flex items-center gap-2 rounded-xl bg-[#ffd45e] px-5 py-3 text-sm font-extrabold disabled:opacity-40">{busy ? <Loader2 size={16} className="animate-spin"/> : <ClipboardCheck size={16}/>}Corrigir agora</button>{officialSource && <a href={officialSource} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-[#241904] px-4 py-3 text-sm font-bold">Abrir prova/manual oficial <ExternalLink size={15}/></a>}{officialKeySource && <a href={officialKeySource} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-[#241904] px-4 py-3 text-sm font-bold">Conferir fonte do gabarito <ExternalLink size={15}/></a>}</div>
      {message && <div className="mt-4 flex gap-2 rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-sm text-[#e0aa18]"><AlertCircle size={17} className="shrink-0 text-[#ffd45e]"/>{message}</div>}

      {analysis && <div className="mt-7 border-t border-[#241904] pt-6">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-5"><div className="col-span-2 rounded-2xl border border-[#6f4f08] bg-[#171105] p-4 md:col-span-1"><BarChart3 className="text-[#ffd45e]" size={20}/><div className="mt-2 text-3xl font-extrabold">{mode === 'answers' ? `${accuracy}%` : '—'}</div><div className="text-xs text-[#e0aa18]">{isEnem ? 'ACERTO BRUTO, SEM TRI' : 'ACERTO OBJETIVO'}</div></div>{[[CheckCircle2, analysis.correct, 'Acertos', 'text-amber-300'], [XCircle, analysis.wrong, 'Erros', 'text-amber-300'], [FileCheck2, analysis.blank, 'Em branco', 'text-amber-200'], [Target, analysis.annulled, 'Anuladas', 'text-amber-200']].map(([Icon, value, label, color]) => { const I = Icon as typeof CheckCircle2; return <div key={String(label)} className="rounded-2xl border border-[#241904] bg-[#0b0904] p-4"><I size={18} className={String(color)}/><div className="mt-2 text-2xl font-extrabold">{String(value)}</div><div className="text-xs text-[#e0aa18]">{String(label)}</div></div>; })}</div>
        {isEnem && mode === 'answers' && <p className="mt-3 text-xs leading-relaxed text-[#b88408]">A quantidade de acertos é exata para o gabarito selecionado. A nota TRI não é estimada: ela depende dos parâmetros dos itens e da coerência do padrão de respostas, não apenas do total de acertos.</p>}

        {(analysis.areaScores.length > 0 || analysis.groups.length > 0) && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-300/20 bg-amber-300/[.05] p-4"><p className="text-sm text-amber-50">{mode === 'answers' ? 'Salve o resultado e envie os erros para o plano adaptativo.' : 'Salve os erros sem alterar suas notas ou percentuais do simulado.'}</p><button onClick={saveDiagnosis} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-amber-300 px-4 py-3 text-sm font-extrabold text-[#0b0904] disabled:opacity-50">{saving ? <Loader2 size={16} className="animate-spin"/> : <Save size={16}/>}Salvar no Plano</button></div>}

        {analysis.areaScores.length > 0 && <div className="mt-6"><h3 className="text-lg font-extrabold">Desempenho por matéria</h3><div className="mt-3 grid gap-3 md:grid-cols-2">{analysis.areaScores.map((score) => { const percent = Math.round(score.correct / Math.max(1, score.total) * 100); return <div key={score.area} className="rounded-2xl border border-[#241904] bg-[#0b0904] p-4"><div className="flex justify-between gap-3"><strong>{score.area}</strong><span className={percent < 60 ? 'text-amber-200' : 'text-amber-300'}>{score.correct}/{score.total} · {percent}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#0b0904]"><span className="block h-full rounded-full bg-[#ffd45e]" style={{ width: `${percent}%` }}/></div></div>; })}</div></div>}

        {analysis.groups.length > 0 && <div className="mt-7"><div><h3 className="text-xl font-extrabold">Mapa dos erros</h3><p className="mt-1 text-sm text-[#e0aa18]">Os pontos mais repetidos aparecem primeiro. A habilidade só é afirmada quando a questão está indexada; caso contrário, usamos apenas a matéria.</p></div>
          <div className="mt-4 grid gap-3">{analysis.groups.map((group, index) => <article key={group.key} className="rounded-2xl border border-[#241904] bg-[#0b0904] p-4 md:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-xs font-extrabold text-[#ffd45e]">PRIORIDADE {index + 1} · {group.numbers.length} ERRO(S)</div><h4 className="mt-1 text-lg font-extrabold">{group.area} · {group.skill}</h4><p className="mt-1 text-xs text-[#e0aa18]">Questões {group.numbers.join(', ')}</p></div><div className="rounded-xl border border-[#6f4f08] bg-[#171105] px-3 py-2 text-xs font-bold">{group.practice.length} semelhantes no banco</div></div><div className="mt-4 grid gap-2 md:grid-cols-3"><div className="rounded-xl border border-[#241904] p-3 text-xs"><b className="text-white">Hoje:</b> refaça as questões sem olhar o gabarito.</div><div className="rounded-xl border border-[#241904] p-3 text-xs"><b className="text-white">Em 7 dias:</b> resolva {Math.max(10, group.numbers.length * 5)} semelhantes.</div><div className="rounded-xl border border-[#241904] p-3 text-xs"><b className="text-white">Em 30 dias:</b> faça novo bloco cronometrado.</div></div></article>)}</div>
        </div>}

        {analysis.rows.some((row) => row.status === 'wrong' || row.status === 'blank') && <details className="mt-6 rounded-2xl border border-[#241904] bg-[#0b0904] p-4"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 font-extrabold">Classificar a causa de cada erro <ChevronDown size={18}/></summary><p className="mt-2 text-xs text-[#e0aa18]">Isso muda a recuperação enviada ao Plano: conteúdo, interpretação, cálculo, desatenção ou tempo.</p><div className="mt-4 grid gap-2">{analysis.rows.filter((row) => row.status === 'wrong' || row.status === 'blank').map((row) => <label key={row.number} className="grid gap-2 rounded-xl border border-[#241904] p-3 text-xs md:grid-cols-[1fr_240px] md:items-center"><span><b>Questão {row.number}</b> · {row.area} · {row.skill}</span><select value={reasons[row.number] ?? 'conteudo'} onChange={(event) => setReasons((current) => ({ ...current, [row.number]: event.target.value }))} className="rounded-lg border border-[#6f4f08] bg-[#0b0904] p-2 text-white">{REASONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>)}</div></details>}
      </div>}
    </div>
  </section>;
}
