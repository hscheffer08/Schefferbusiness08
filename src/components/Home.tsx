  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 py-6 md:px-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={country === 'US'}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-sm font-semibold transition-colors ${country === 'US' ? 'cursor-not-allowed border-ink-800 bg-ink-900/50 text-ink-600' : 'border-amber-500/40 bg-amber-500/10 hover:bg-amber-500/20 text-amber-200'}`}
      ...
      <main className="relative z-10 flex flex-col items-center justify-center px-6 pt-8 pb-24 md:pt-16 md:pb-32 max-w-5xl mx-auto text-center">
        ...
        {country === 'BR' && (
          <button onClick={() => onNavigate('vocational-demo')} className="animate-fade-up mt-5 w-full max-w-3xl group relative overflow-hidden rounded-2xl border border-amber-500/35 bg-gradient-to-r from-amber-500/10 via-brand-500/5 to-accent-500/10 p-5 md:p-6 text-left transition-all hover:border-amber-400/60 hover:scale-[1.01]" style={{ animationDelay: '0.25s' }}>
            <div className="relative flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-amber-500/15 text-amber-300 flex items-center justify-center flex-shrink-0"><Compass className="w-6 h-6" /></div>
              <div className="flex-1"><div className="flex flex-wrap items-center gap-2 mb-1"><h3 className="text-lg font-bold text-ink-50">Demo de Exploração Vocacional</h3><span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-300 text-[11px] font-bold">NOVO · DEMO</span></div><p className="text-sm text-ink-400 leading-relaxed">Descubra quais áreas e cursos combinam com seus interesses, aptidões percebidas, valores e estilo de trabalho. 48 perguntas · 50 cursos.</p></div>
              <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-amber-300 group-hover:gap-2.5 transition-all">Fazer teste vocacional <ArrowRight className="w-4 h-4" /></span>
            </div>
          </button>
        )}

        <button onClick={() => onNavigate('faculty-questionnaire')} disabled={country === 'US'} className={`animate-fade-up mt-5 w-full max-w-3xl group relative overflow-hidden rounded-2xl border p-5 md:p-6 text-left transition-all ${country === 'US' ? 'cursor-not-allowed border-ink-800 bg-ink-900/40 opacity-55' : 'border-amber-500/35 bg-gradient-to-r from-amber-500/10 via-brand-500/5 to-accent-500/10 hover:border-amber-400/60'}`} style={{ animationDelay: '0.3s' }}>
          <div className="relative flex flex-col sm:flex-row sm:items-center gap-4"><div className="w-12 h-12 rounded-2xl bg-amber-500/15 text-amber-300 flex items-center justify-center flex-shrink-0"><FileText className="w-6 h-6" /></div><div className="flex-1"><div className="flex flex-wrap items-center gap-2 mb-1"><h3 className="text-lg font-bold text-ink-50">Questionário para as Faculdades</h3><span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-ink-800 text-ink-400 text-[11px] font-medium"><LockKeyhole className="w-3 h-3" /> {country === 'US' ? 'Somente Brasil' : 'Requer conta'}</span></div><p className="text-sm text-ink-400 leading-relaxed">{country === 'US' ? 'Este hub exclusivo para relacionamento com faculdades está disponível apenas na experiência brasileira.' : 'Organize notas, extracurriculares, idiomas, conquistas, projetos e experiências em um só lugar.'}</p></div><span className={`inline-flex items-center gap-1.5 text-sm font-semibold transition-all ${country === 'US' ? 'text-ink-600' : 'text-amber-300 group-hover:gap-2.5'}`}>{country === 'US' ? 'Indisponível nos EUA' : 'Abrir meu perfil'} {country === 'BR' ? <ArrowRight className="w-4 h-4" /> : null}</span></div>
        </button>
...
function BrazilFlag() {
  return <svg aria-hidden="true" focusable="false" viewBox="0 0 28 20" className="h-10 w-14 overflow-hidden rounded-md shadow-md ring-1 ring-white/20"><rect width="28" height="20" fill="#6f4f08" /><path d="M14 2.2 25.2 10 14 17.8 2.8 10Z" fill="#ffd45e" /><circle cx="14" cy="10" r="4.25" fill="#241904" /><path d="M9.9 9.15c2.8-.7 5.8-.25 8.25 1.3" fill="none" stroke="#fffaf0" strokeWidth=".65" /></svg>;
}

function UnitedStatesFlag() {
  return <svg aria-hidden="true" focusable="false" viewBox="0 0 39 26" className="h-10 w-14 overflow-hidden rounded-md shadow-md ring-1 ring-white/20"><rect width="39" height="26" fill="#fffaf0" />{[0, 4, 8, 12, 16, 20, 24].map((y) => <rect key={y} y={y} width="39" height="2" fill="#b88408" />)}<rect width="15.6" height="14" fill="#241904" />{Array.from({ length: 9 }, (_, row) => <g key={row}>{Array.from({ length: row % 2 === 0 ? 6 : 5 }, (__, column) => <circle key={column} cx={(row % 2 === 0 ? 1.25 : 2.5) + column * 2.55} cy={0.8 + row * 1.5} r="0.34" fill="#fffaf0" />)}</g>)}</svg>;
}
