    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 py-6 md:px-12">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-lg tracking-tight">B-School<span className="text-brand-400"> Fit</span></span>
        </div>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center px-6 pt-8 pb-24 md:pt-16 md:pb-32 max-w-5xl mx-auto text-center">
        ...
        <div className="mt-12 max-w-2xl glass rounded-2xl border border-ink-800 p-5 text-left"><h3 className="font-semibold text-ink-200 text-sm mb-1.5">🎓 Período de lançamento</h3><p className="text-sm text-ink-400 leading-relaxed">Faça seu B-School Fit gratuitamente. O acesso será pago futuramente, pelo valor de R$8.</p></div>
      </main>

      <footer className="relative z-10 border-t border-ink-800/50 px-6 py-8 md:px-12">
        <div className="max-w-5xl mx-auto"><div className="flex flex-col sm:flex-row items-center justify-between gap-4"><div className="flex items-center gap-2.5"><div className="w-7 h-7 rounded-lg bg-gradient-to-br from-brand-400 to-brand-600 flex items-center justify-center"><GraduationCap className="w-4 h-4 text-ink-950" strokeWidth={2.5} /></div><span className="font-bold text-sm tracking-tight">B-School Fit</span></div><div className="flex flex-wrap items-center justify-center gap-4 text-xs text-ink-500"><button onClick={() => onNavigate('howitworks')} className="flex items-center gap-1 hover:text-ink-300 transition-colors"><BookOpen className="w-3.5 h-3.5" /> Como funciona</button><button onClick={() => onNavigate('methodology')} className="flex items-center gap-1 hover:text-ink-300 transition-colors"><FlaskConical className="w-3.5 h-3.5" /> Metodologia</button><button onClick={() => onNavigate('faq')} className="flex items-center gap-1 hover:text-ink-300 transition-colors"><HelpCircle className="w-3.5 h-3.5" /> FAQ</button><button onClick={() => onNavigate('privacy')} className="flex items-center gap-1 hover:text-ink-300 transition-colors"><Shield className="w-3.5 h-3.5" /> Privacidade</button><button onClick={() => onNavigate('terms')} className="flex items-center gap-1 hover:text-ink-300 transition-colors"><FileText className="w-3.5 h-3.5" /> Termos</button></div></div><p className="text-center text-xs text-ink-600 mt-4">B-School Fit mede compatibilidade de perfil, não chance de aprovação. Feito para estudantes brasileiros.</p></div>
      </footer>
    </div>
  );
}
