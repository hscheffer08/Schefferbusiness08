...
export default function OfficialQuestionWorkspaceV3(){
  ...
  return <section className="pb-8">
    ...
    {years.length>1&&<div className="mt-2 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{years.map(v=><button key={v} onClick={()=>{setYear(v);setSubject('Todas')}} className={`shrink-0 rounded-lg border px-3 py-1.5 text-[11px] font-bold ${year===v?'border-[#ffd45e] bg-[#241904] text-white':'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>{v==='Todos'?'Todos os anos':v}</button>)}</div>}
    ...
    <div className="mt-3 grid gap-2.5 md:grid-cols-2">{filtered.slice(0,visibleCount).map((q:any)=>mode==='official'?<button key={q.question_id} onClick={()=>loadOfficial(q)} className="rounded-2xl border border-amber-400/20 bg-[#0b0904] p-4 text-left"><div className="flex items-center justify-between"><span className="rounded-full bg-amber-300/10 px-2 py-1 text-[9px] font-black uppercase tracking-wide text-amber-200">Oficial</span><span className="text-[10px] text-[#b88408]">{q.year}</span></div><div className="mt-3 text-[11px] font-bold text-[#ffd45e]">{edition(q,cfg.label)}</div><strong className="mt-1.5 block text-sm">{officialSubject(q,enemSubjectMaps)||q.skill_name||q.subject||q.area||'Questão oficial'}</strong><p className="mt-2 text-xs text-[#e0aa18]">Abrir e responder dentro do site</p></button>:<button key={q.id} onClick={()=>openPractice(q)} className="rounded-2xl border border-[#241904] bg-[#0b0904] p-4 text-left"><div className="text-[10px] text-[#b88408]">nível {q.difficulty}/5</div><strong className="mt-2 block text-sm">{q.skill_name}</strong><p className="mt-2 line-clamp-2 text-xs text-[#e0aa18]">{q.prompt}</p></button>)}</div>
