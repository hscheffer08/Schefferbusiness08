...
function officialSubject(q:OfficialRef){
...
export default function OfficialQuestionWorkspaceV3(){
  ...
  useEffect(()=>{localStorage.setItem('conectae:active-exam',exam);setArea('Todas');setSubject('Todas');setYear('Todos');setSearch('');setMode(cfg.vestibular?'official':'authorial')},[exam,cfg.vestibular]);
  ...
  const availableSubjects=useMemo(()=>{
    ...
    return Array.from(new Set(base.map(officialSubject).filter(Boolean))).sort((a,b)=>a.localeCompare(b,'pt-BR'));
  },[official,area,mode]);
  ...
  const filtered=useMemo(()=>rows.filter((q:any)=>{if(area!=='Todas'&&q.area!==area)return false;if(mode==='official'&&subject!=='Todas'&&officialSubject(q)!==subject)return false;const y=mode==='official'?q.year:q.source_exam_year;if(year!=='Todos'&&String(y)!==year)return false;const t=search.trim().toLowerCase();if(!t)return true;const h=mode==='official'?`${q.area||''} ${officialSubject(q)} ${q.subject||''} ${q.skill_name||''} ${q.question_number} ${q.year} ${q.series_id==='cmmg'?semester(q):''}`:`${q.area||''} ${q.skill_name||''} ${q.prompt||''} ${q.source_question_number||''}`;return h.toLowerCase().includes(t)}),[rows,area,subject,year,search,mode]);
  const counts=useMemo(()=>Object.fromEntries(cfg.areas.map(a=>[a,official.filter(q=>q.area===a).length])),[official,cfg.areas]);
  const subjectCounts=useMemo(()=>Object.fromEntries(availableSubjects.map(s=>[s,official.filter(q=>(area==='Todas'||q.area===area)&&officialSubject(q)===s).length])),[official,area,availableSubjects]);
  ...
  return <section className="pb-8">
    <div className="mb-4"><div className="text-[11px] font-extrabold uppercase tracking-[.1em] text-[#ffd45e]">Banco de questões</div><h1 className="mt-1 text-2xl font-extrabold tracking-[-.035em] md:text-3xl">Questões oficiais para resolver aqui.</h1><p className="mt-2 max-w-2xl text-sm text-[#e0aa18]">Escolha o vestibular, o componente e, quando disponível, a matéria específica. A seção “Oficiais” contém somente itens de provas reais.</p></div>
    <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{EXAMS.map(x=><button key={x.id} onClick={()=>setExam(x.id)} className={`shrink-0 rounded-xl border px-4 py-2.5 text-sm font-extrabold ${exam===x.id?'border-[#ffd45e] bg-[#ffd45e] text-[#050505]':'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>{x.label}</button>)}</div>
    <div className="mt-3 grid grid-cols-3 gap-2 rounded-2xl border border-[#241904] bg-[#0b0904] p-2"><button disabled={!cfg.vestibular} onClick={()=>changeMode('official')} className={`rounded-xl px-2 py-3 text-xs font-extrabold disabled:opacity-35 ${mode==='official'?'bg-amber-500/15 text-amber-200':'text-[#e0aa18]'}`}>Oficiais<span className="block text-lg">{official.length}</span></button><button onClick={()=>changeMode('adapted')} className={`rounded-xl px-2 py-3 text-xs font-extrabold ${mode==='adapted'?'bg-[#171105] text-white':'text-[#e0aa18]'}`}>Adaptadas<span className="block text-lg">{adapted.length}</span></button><button onClick={()=>changeMode('authorial')} className={`rounded-xl px-2 py-3 text-xs font-extrabold ${mode==='authorial'?'bg-[#171105] text-white':'text-[#e0aa18]'}`}>Estilo da prova<span className="block text-lg">{authorial.length}</span></button></div>
    {mode==='official'&&cfg.vestibular&&<div className="mt-3 rounded-2xl border border-amber-400/25 bg-amber-400/[.05] p-4"><div className="font-extrabold">{cfg.label} · acervo oficial interativo</div><div className="mt-3 flex gap-2 overflow-x-auto pb-1">{cfg.areas.map(a=><button key={a} onClick={()=>chooseArea(a)} className={`shrink-0 rounded-xl border px-3 py-2 text-left ${area===a?'border-amber-300/50 bg-amber-300/10':'border-[#241904] bg-[#0b0904]'}`}><strong className="block text-sm">{a}</strong><span className="text-[10px] text-[#e0aa18]">{counts[a]||0} oficiais</span></button>)}</div>{availableSubjects.length>0&&<div className="mt-4 border-t border-amber-300/10 pt-4"><div className="text-[10px] font-black uppercase tracking-[.12em] text-amber-200">Matéria específica</div><div className="mt-2 flex flex-wrap gap-2"><button onClick={()=>setSubject('Todas')} className={`rounded-lg border px-3 py-2 text-xs font-bold ${subject==='Todas'?'border-[#ffd45e] bg-[#241904]':'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>Todas</button>{availableSubjects.map(s=><button key={s} onClick={()=>setSubject(s)} className={`rounded-lg border px-3 py-2 text-xs font-bold ${subject===s?'border-[#ffd45e] bg-[#241904]':'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>{s} <span className="opacity-70">({subjectCounts[s]||0})</span></button>)}</div></div>}</div>}
    <div className="mt-3 flex flex-wrap gap-2"><button onClick={()=>chooseArea('Todas')} className={`rounded-lg border px-3 py-2 text-xs font-bold ${area==='Todas'?'border-[#ffd45e] bg-[#241904]':'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>Todas as áreas</button>{availableAreas.map(a=><button key={a} onClick={()=>chooseArea(a)} className={`rounded-lg border px-3 py-2 text-xs font-bold ${area===a?'border-[#ffd45e] bg-[#241904]':'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>{a}</button>)}</div>
    <div className="relative mt-3"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#b88408]" size={16}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar conteúdo, matéria ou número" className="h-11 w-full rounded-xl border border-[#241904] bg-[#0b0904] pl-10 pr-3 text-sm text-white outline-none placeholder:text-[#c7b987]"/></div>
    {years.length>1&&<div className="mt-2 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{years.map(v=><button key={v} onClick={()=>setYear(v)} className={`shrink-0 rounded-lg border px-3 py-1.5 text-[11px] font-bold ${year===v?'border-[#ffd45e] bg-[#241904] text-white':'border-[#241904] bg-[#0b0904] text-[#e0aa18]'}`}>{v==='Todos'?'Todos os anos':v}</button>)}</div>}
    ...
    <div className="mt-4 text-xs font-bold text-[#e0aa18]">{filtered.length} {filtered.length===1?'questão encontrada':'questões encontradas'}{mode==='official'&&<span className="ml-1 text-amber-200">· somente oficiais</span>}</div>
    <div className="mt-3 grid gap-2.5 md:grid-cols-2">{filtered.slice(0,visibleCount).map((q:any)=>mode==='official'?<button key={q.question_id} onClick={()=>loadOfficial(q)} className="rounded-2xl border border-amber-400/20 bg-[#0b0904] p-4 text-left"><div className="flex items-center justify-between"><span className="rounded-full bg-amber-300/10 px-2 py-1 text-[9px] font-black uppercase tracking-wide text-amber-200">Oficial</span><span className="text-[10px] text-[#b88408]">{q.year}</span></div><div className="mt-3 text-[11px] font-bold text-[#ffd45e]">{edition(q,cfg.label)}</div><strong className="mt-1.5 block text-sm">{officialSubject(q)||q.skill_name||q.subject||q.area||'Questão oficial'}</strong><p className="mt-2 text-xs text-[#e0aa18]">Abrir e responder dentro do site</p></button>:<button key={q.id} onClick={()=>openPractice(q)} className="rounded-2xl border border-[#241904] bg-[#0b0904] p-4 text-left"><div className="text-[10px] text-[#b88408]">nível {q.difficulty}/5</div><strong className="mt-2 block text-sm">{q.skill_name}</strong><p className="mt-2 line-clamp-2 text-xs text-[#e0aa18]">{q.prompt}</p></button>)}</div>
    {visibleCount<filtered.length&&<button onClick={()=>setVisibleCount(v=>v+120)} className="mt-4 min-h-11 w-full rounded-xl border border-[#ffd45e] bg-[#171105] px-4 text-sm font-extrabold">Carregar mais questões</button>}
    ...
    {modalOpen&&<div className="fixed inset-0 z-[220] overflow-y-auto bg-[#050505] text-white" role="dialog" aria-modal="true"><div className="mx-auto min-h-full w-full max-w-3xl px-4 pb-[calc(env(safe-area-inset-bottom)+28px)] pt-[max(12px,env(safe-area-inset-top))] md:px-6"><div className="sticky top-0 z-10 -mx-1 flex items-center justify-between gap-3 border-b border-[#241904] bg-[#050505]/97 px-1 py-3 backdrop-blur-xl"><div><div className={`text-[11px] font-black uppercase tracking-wide ${activeOfficial?'text-amber-200':'text-[#ffd45e]'}`}>{activeOfficial?'Questão oficial':mode==='adapted'?'Adaptada de prova real':'Estilo da prova'}</div><div className="text-sm font-extrabold">{activeOfficial?edition(activeOfficial,cfg.label):cfg.label}</div></div><button onClick={close} className="grid h-11 w-11 place-items-center rounded-xl border border-[#241904] bg-[#0b0904]" aria-label="Fechar"><X size={20}/></button></div>
      <div className="py-5">{extracting&&<div className="grid min-h-[260px] place-items-center"><div className="text-center"><Loader2 className="mx-auto animate-spin text-[#ffd45e]"/><p className="mt-3 text-sm text-[#e0aa18]">Lendo a questão da prova oficial…</p></div></div>}
      {!extracting&&extractError&&<div className="rounded-2xl border border-amber-400/25 bg-amber-400/[.06] p-5"><strong>Não consegui reconstruir a questão nesta tentativa.</strong><p className="mt-2 text-sm text-[#e0aa18]">{extractError}</p><button onClick={retry} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#ffd45e] px-4 text-sm font-extrabold text-[#050505]"><RotateCcw size={16}/>Tentar novamente</button>{activeOfficial?.source_pdf_url&&<a href={activeOfficial.source_pdf_url} target="_blank" rel="noreferrer" className="ml-3 mt-4 inline-flex items-center gap-2 text-sm font-bold text-[#ffd45e]"><ExternalLink size={15}/>Fonte oficial</a>}</div>}
      {!extracting&&!extractError&&prompt&&source&&<><div className="text-xs font-bold text-[#ffd45e]">{activeOfficial?(officialSubject(activeOfficial)||activeOfficial.area):activePractice?.area}</div><h2 className="mt-3 whitespace-pre-line text-lg font-extrabold leading-relaxed md:text-xl">{prompt}</h2>{extracted?.images?.map((url,index)=><img key={url} src={url} alt={`Elemento visual ${index+1} da questão`} loading="lazy" className="mt-4 max-h-[560px] w-full rounded-xl border border-[#241904] bg-white object-contain"/>)}{activeOfficial&&extracted?.needs_source_image&&<div className="mt-3 rounded-xl border border-amber-300/25 bg-amber-300/[.06] p-3 text-xs text-[#e0aa18]">{extracted.image_note||'Confira o elemento visual original antes de responder.'}</div>}{activeOfficial?.source_pdf_url&&extracted?.needs_source_image&&!extracted.images?.length&&<iframe title="Página original da questão" src={`/api/proxy-official-pdf?url=${encodeURIComponent(activeOfficial.source_pdf_url)}#page=${Math.max(1,(extracted.source_page||1)-1)}`} className="mt-4 h-[560px] w-full rounded-xl border border-[#241904] bg-white"/>}<div className="mt-5 grid gap-2.5">{LETTERS.map(l=>{const text=option(source,l);if(!text)return null;const chosen=selected===l,isCorrect=submitted&&correct===l,isWrong=submitted&&chosen&&correct!==l;const optionImage=extracted?.option_images?.[l];return <button key={l} disabled={submitted} onClick={()=>setSelected(l)} className={`flex min-h-14 items-start gap-3 rounded-xl border px-4 py-3 text-left ${isCorrect?'border-amber-400 bg-amber-400/10':isWrong?'border-amber-400 bg-amber-400/10':chosen?'border-[#ffd45e] bg-[#241904]':'border-[#241904] bg-[#0b0904]'}`}><strong>{l}</strong><span className="min-w-0 flex-1 text-sm leading-relaxed">{text}{optionImage&&<img src={optionImage} alt={`Imagem da alternativa ${l}`} loading="lazy" className="mt-2 max-h-72 w-full rounded-lg bg-white object-contain"/>}</span></button>})}</div>
      {!submitted?<button disabled={!selected||answering} onClick={activeOfficial?submitOfficial:submitPractice} className="mt-5 min-h-12 w-full rounded-xl bg-[#ffd45e] px-4 text-sm font-extrabold text-[#050505] disabled:opacity-40">{answering?'Conferindo gabarito…':'Confirmar resposta'}</button>:<div className="mt-5 rounded-2xl border border-amber-400/30 bg-amber-400/[.07] p-4"><div className="flex items-center gap-2 font-extrabold">{correct?(result?<><CheckCircle2 size={19}/>Resposta correta</>:<><XCircle size={19}/>Resposta incorreta</>):'Gabarito temporariamente indisponível'}</div>{correct&&<p className="mt-2 text-sm">Gabarito: <strong>{correct}</strong></p>}{activePractice?.explanation&&<p className="mt-2 text-sm text-[#e0aa18]">{activePractice.explanation}</p>}{activeOfficial?.source_pdf_url&&<a href={activeOfficial.source_pdf_url} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 text-xs font-bold text-[#ffd45e]"><ExternalLink size={14}/>Conferir prova oficial</a>}</div>}</>}
      </div></div></div>}
  </section>;
}
