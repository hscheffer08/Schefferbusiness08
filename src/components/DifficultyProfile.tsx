 ...
 return <section className="plan6-card span12" style={{overflow:'hidden'}}>
  ...
  <div className="plan6-callout blue" style={{margin:'4px 0 14px'}}>
   ...
   <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:8,marginTop:12}}>
    {subjectStats.map(s=><button key={s.subject} type="button" onClick={()=>openSubject(s.subject)} style={{border:`1px solid ${s.marked?'rgba(114,165,255,.55)':'rgba(113,147,198,.16)'}`,borderRadius:14,padding:'12px',background:s.marked?'rgba(36,108,255,.06)':'rgba(255,255,255,.015)',textAlign:'left'}}>
  ...
  {started&&<>
   ...
   <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,flexWrap:'wrap',marginTop:16,paddingTop:14,borderTop:'1px solid rgba(113,147,198,.14)'}}><div style={{fontSize:12,opacity:.62}}>Base: {catalog.label} · o gêmeo salva matéria + conteúdo + intensidade.</div><button className="plan6-btn primary" onClick={save} disabled={saving||selectedDetails.length===0}><Save size={15}/>{saving?'Salvando...':selectedDetails.length?'Salvar dificuldades específicas':'Escolha um conteúdo'}</button></div>
  </>}
