...
export default function AdmissionsPlanner({ onBack }: AdmissionsPlannerProps) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#050505]"><Loader2 className="w-8 h-8 animate-spin text-amber-300" /></div>;

  return (
    <div className="min-h-screen bg-[#050505] text-ink-50">
      <header className="max-w-7xl mx-auto px-5 md:px-8 py-6 flex items-center justify-between">
        ...
        <span className="rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1.5 text-xs font-bold text-amber-200">Plano de Aprovação</span>
      </header>

      <main className="max-w-7xl mx-auto px-5 md:px-8 pb-24">
        <section className="mb-7">
          <h1 className="text-4xl md:text-6xl font-black tracking-tight">Seu plano muda quando <span className="text-amber-300">seu desempenho muda.</span></h1>
        ...
        <section className="grid lg:grid-cols-2 gap-5 mb-5">
          <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-5">
            ...
            <div className="mt-5 rounded-2xl border border-amber-300/25 bg-amber-300/[0.07] p-5">
              <p className="text-xs font-black uppercase tracking-widest text-amber-300">Sua meta final</p>
              <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><div className="text-5xl font-black text-white">{targetScore}</div><p className="text-xs text-ink-500">referência competitiva geral</p></div><div className="text-right"><strong className="text-amber-200">{displayExam}</strong><p className="text-xs text-ink-500">{days} dias · {weeks} semanas</p></div></div>
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-5 gap-2">{([...OBJECTIVE_AREAS, 'essay'] as EnemArea[]).map((a) => <div key={a} className="rounded-xl bg-black/20 p-2.5 text-center"><p className="text-[10px] font-bold text-ink-500">{AREA_SHORT[a]}</p><p className="font-black text-amber-100">{plan.targets[a]}{a === 'essay' ? '/1000' : '/45'}</p></div>)}</div>
            </div>
          </div>
          ...
          <div className="rounded-3xl border-2 border-amber-300/25 bg-gradient-to-br from-amber-300/[0.09] to-transparent p-5 shadow-xl shadow-amber-950/10">
            <div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-amber-300 text-[#050505] flex items-center justify-center"><Sparkles className="w-5 h-5" /></div><div><p className="text-xs font-black uppercase tracking-widest text-amber-200">2 · Importante</p><h2 className="text-xl font-black">Quais matérias são mais confortáveis para você?</h2></div></div>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 mb-6">
          <div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-widest text-ink-500">3 · Novo simulado</p><h2 className="text-xl font-black mt-1">Atualize o que você realmente está acertando</h2></div><TrendingUp className="w-5 h-5 text-amber-300" /></div>
          <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-5 gap-3">{OBJECTIVE_AREAS.map((a) => <label key={a} className="rounded-2xl border border-white/10 bg-black/15 p-3"><span className="text-xs text-ink-400">{AREA_LABEL[a]}</span><div className="mt-1 flex items-end justify-between"><strong className="text-2xl">{draftScores[a]}</strong><span className="text-xs text-amber-300">meta {plan.targets[a]}</span></div><input type="range" min={0} max={45} value={draftScores[a]} onChange={(e) => setDraftScores((s) => ({ ...s, [a]: Number(e.target.value) }))} className="w-full mt-3" /></label>)}<label className="rounded-2xl border border-white/10 bg-black/15 p-3"><span className="text-xs text-ink-400">Redação</span><div className="mt-1 flex items-end justify-between"><strong className="text-2xl">{draftScores.essay}</strong><span className="text-xs text-amber-300">meta {plan.targets.essay}</span></div><input type="range" min={0} max={1000} step={20} value={draftScores.essay} onChange={(e) => setDraftScores((s) => ({ ...s, essay: Number(e.target.value) }))} className="w-full mt-3" /></label></div>
          <button onClick={registerSimulation} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-amber-300 px-5 py-3 font-black text-[#050505]"><RefreshCw className="w-4 h-4" />Registrar simulado e recalcular tudo</button>
        </section>

        <nav className="flex gap-2 overflow-x-auto mb-5">{([['goal','Metas',Target],['plan','Plano semanal',CalendarDays],['practice','Questões',BookOpen],['scanner','Scanner',ScanLine]] as const).map(([id,label,Icon]) => <button key={id} onClick={() => setTab(id)} className={`whitespace-nowrap rounded-xl border px-4 py-2.5 text-sm font-bold inline-flex items-center gap-2 ${tab === id ? 'border-amber-300/40 bg-amber-300/10 text-amber-200' : 'border-white/10 text-ink-400'}`}><Icon className="w-4 h-4" />{label}</button>)}</nav>

        {tab === 'goal' && <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-5"><div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">{([...OBJECTIVE_AREAS, 'essay'] as EnemArea[]).map((a) => { const now = scores[a], target = plan.targets[a], max = a === 'essay' ? 1000 : 45; return <div key={a} className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-black text-ink-500">{AREA_LABEL[a]}</p><p className="mt-2 text-3xl font-black text-amber-200">{target}<span className="text-xs text-ink-600">/{max}</span></p><p className="mt-2 text-xs text-ink-400">Atual {now} · {target > now ? `faltam ${target-now}` : 'meta atingida'}</p></div>; })}</div>{simulations.length > 0 && <div className="mt-5 text-xs text-ink-500">Último recálculo: {lastPlanUpdate || new Date(simulations[0].createdAt).toLocaleDateString('pt-BR')}</div>}</section>}

        {tab === 'plan' && <section className="space-y-4">
          <div className="rounded-3xl border border-amber-300/20 bg-amber-300/[0.05] p-5 flex flex-col md:flex-row md:items-center justify-between gap-4"><div><h2 className="text-xl font-black">Plano adaptativo até {displayExam}</h2><p className="text-sm text-ink-400">Cada novo simulado recria os checkpoints e os materiais sugeridos.</p></div><button onClick={() => generatePdf(1)} className="rounded-xl border border-amber-300/30 px-4 py-2.5 text-sm font-bold text-amber-200 inline-flex items-center gap-2"><Download className="w-4 h-4" />PDF do plano</button></div>
          {plan.rows.map((row) => {
            ...
            return <div key={row.week} className="rounded-3xl border border-white/10 bg-white/[0.035] p-5">
              <div className="flex flex-col lg:flex-row lg:items-center gap-4"><div className="lg:w-28"><p className="text-xs text-ink-500">CHECKPOINT</p><h3 className="text-xl font-black">Semana {row.week}</h3></div><div className="grid grid-cols-2 sm:grid-cols-5 gap-2 flex-1">{row.objective.map(({area:a,hits}) => <div key={a} className="rounded-xl bg-black/20 p-2.5 text-center"><p className="text-[10px] text-ink-500 font-bold">{AREA_SHORT[a]}</p><p className="font-black">{hits}/45</p></div>)}<div className="rounded-xl bg-amber-300/[0.07] p-2.5 text-center"><p className="text-[10px] text-amber-400 font-bold">RED</p><p className="font-black text-amber-100">{row.essay}</p></div></div></div>
              <div className="mt-4 grid md:grid-cols-3 gap-3">
                ...
                <button onClick={() => { setQuestionIndex(Math.max(0, currentQuestions.findIndex((q) => weeklyQuestions.some((x) => x.id === q.id)))); setTab('practice'); }} className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.06] p-4 text-left hover:border-amber-300/40"><BookOpen className="w-5 h-5 text-amber-300" /><strong className="block mt-2">Fazer questões agora</strong><span className="text-xs text-ink-500">Prática alinhada à prova e às habilidades da semana.</span></button>
                <div className="grid grid-cols-2 gap-2"><button onClick={() => openVideo(row.focus[0])} className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.05] p-3 text-left"><PlayCircle className="w-5 h-5 text-amber-300" /><strong className="block mt-2 text-sm">Vídeo</strong><span className="text-[11px] text-ink-500">Buscar aula da habilidade</span></button><button onClick={() => generatePdf(row.week)} className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.05] p-3 text-left"><FileText className="w-5 h-5 text-amber-300" /><strong className="block mt-2 text-sm">PDF</strong><span className="text-[11px] text-ink-500">Semana + questões</span></button></div>
              </div>
            </div>;
          })}
          {currentResources.length > 0 && <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-5"><h3 className="font-black">Fontes e materiais da prova</h3><div className="mt-3 grid md:grid-cols-2 gap-3">{currentResources.slice(0,6).map((r) => <a key={r.id} href={r.url || `https://www.youtube.com/results?search_query=${encodeURIComponent(r.search_query || '')}`} target="_blank" rel="noreferrer" className="rounded-2xl border border-white/10 bg-black/15 p-4 hover:border-amber-300/30"><div className="flex items-center justify-between"><strong className="text-sm">{r.title}</strong><ExternalLink className="w-4 h-4 text-amber-300" /></div><p className="mt-1 text-xs text-ink-500">{r.description}</p></a>)}</div></div>}
        </section>}

        {tab === 'practice' && <section className="grid lg:grid-cols-[1.4fr_.6fr] gap-5">
          <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 md:p-7">{practice ? <><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-amber-300/10 border border-amber-300/20 px-3 py-1 text-xs font-bold text-amber-200">{practice.area} · {practice.skill_name}</span><span className="text-xs text-ink-500">Dificuldade {practice.difficulty}/5</span></div><h2 className="mt-5 text-xl md:text-2xl font-black leading-relaxed">{practice.prompt}</h2>{practice.option_a ? <div className="mt-5 space-y-2">{(['A','B','C','D','E'] as const).map((letter) => { const value = practice[`option_${letter.toLowerCase()}` as keyof PracticeQuestion] as string | null; if (!value) return null; const chosen = selectedAnswer === letter; return <button key={letter} onClick={() => { setSelectedAnswer(letter); setShowCorrection(false); }} className={`w-full rounded-2xl border p-4 text-left ${chosen ? 'border-amber-300 bg-amber-300/10' : 'border-white/10 bg-black/15'}`}><strong className="mr-3 text-amber-300">{letter}</strong>{value}</button>; })}</div> : <textarea className="mt-5 w-full min-h-36 rounded-2xl border border-white/10 bg-black/20 p-4" placeholder="Escreva sua resposta discursiva antes de ver a correção." />}
          <div className="mt-5 flex flex-wrap gap-2"><button onClick={() => setShowCorrection(true)} className="rounded-xl bg-amber-300 px-5 py-3 font-black text-[#050505]">Corrigir</button><button onClick={nextQuestion} className="rounded-xl border border-white/15 px-5 py-3 font-bold">Próxima questão <ArrowRight className="w-4 h-4 inline ml-1" /></button><button onClick={() => generatePdf(1)} className="rounded-xl border border-amber-300/25 px-4 py-3 text-amber-200 font-bold"><Download className="w-4 h-4 inline mr-1" />PDF</button></div>{showCorrection && <div className={`mt-5 rounded-2xl border p-4 ${!practice.correct_option || selectedAnswer === practice.correct_option ? 'border-amber-300/20 bg-amber-300/[0.05]' : 'border-amber-300/20 bg-amber-300/[0.05]'}`}><strong>{practice.correct_option ? `Resposta: ${practice.correct_option}` : 'Resposta esperada'}</strong><p className="mt-2 text-sm text-ink-300">{practice.explanation}</p></div>}</> : <div className="text-center py-16 text-ink-500">Ainda não há questões autorais para esta prova.</div>}</div>
          <aside className="rounded-3xl border border-white/10 bg-white/[0.035] p-5"><Brain className="w-7 h-7 text-amber-300" /><h3 className="mt-3 text-lg font-black">Banco da prova</h3><p className="mt-2 text-sm text-ink-400">{currentQuestions.length} questões autorais disponíveis nesta versão, conectadas à taxonomia da prova.</p><button onClick={() => practice && openVideo(practice.skill_name)} className="mt-4 w-full rounded-xl border border-amber-300/20 bg-amber-300/[0.05] p-3 font-bold text-amber-200"><PlayCircle className="w-4 h-4 inline mr-2" />Ver aula desta habilidade</button></aside>
        </section>}

        {tab === 'scanner' && <section className="grid lg:grid-cols-2 gap-5"><div className="rounded-3xl border border-white/10 bg-white/[0.035] p-5"><h2 className="text-xl font-black">Escaneie uma questão</h2><input ref={fileRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => void handleImage(e.target.files?.[0])} /><button onClick={() => fileRef.current?.click()} className="mt-4 w-full min-h-40 rounded-2xl border border-dashed border-white/15 bg-black/15 overflow-hidden">{imageUrl ? <img src={imageUrl} className="max-h-64 mx-auto object-contain" alt="Questão" /> : <div className="text-ink-500"><Camera className="w-8 h-8 mx-auto mb-2" />Tirar foto ou enviar imagem</div>}</button><textarea value={questionText} onChange={(e) => setQuestionText(e.target.value)} className="mt-4 w-full min-h-32 rounded-2xl border border-white/10 bg-black/20 p-4" placeholder="Cole ou digite o enunciado para classificar a habilidade." /><button onClick={analyzeQuestion} className="mt-3 rounded-xl bg-amber-300 px-5 py-3 font-black text-[#050505]">Analisar habilidade</button></div><div className="rounded-3xl border border-white/10 bg-white/[0.035] p-5">{scanResult ? <><span className="rounded-full bg-amber-300/10 px-3 py-1 text-xs font-bold text-amber-200">{AREA_LABEL[scanResult.area]}</span><h3 className="mt-4 text-2xl font-black">{scanResult.skill}</h3><p className="mt-2 text-sm text-ink-400">Use esta classificação para buscar prática e vídeo. O próximo passo recomendado é fazer 5 questões do mesmo tipo e repetir em 48 horas.</p><button onClick={() => { setTab('practice'); setQuestionIndex(0); }} className="mt-5 rounded-xl border border-amber-300/25 px-4 py-3 text-amber-200 font-bold">Ir para questões</button></> : <div className="h-full min-h-72 flex items-center justify-center text-center text-ink-500"><div><ScanLine className="w-10 h-10 mx-auto mb-3" />O diagnóstico aparece aqui.</div></div>}</div></section>}
      </main>
    </div>
  );
}
