...
import { BarChart3, BookOpenCheck, BrainCircuit, ChevronLeft, FileText, Home, LayoutGrid, Loader2, LogOut, Mic2, ScanLine, Target } from 'lucide-react';
...
function Gate({ onBack }: { onBack: () => void }) {
  ...
  useEffect(()=>{
    if(!loading&&!user){window.location.replace('/?auth=login&next=course');}
  },[loading,user]);

  if (loading || !user) return <div className="min-h-screen bg-[#020817] flex items-center justify-center text-[#72a5ff]"><Loader2 className="w-8 h-8 animate-spin" /></div>;
