...
function Gate({ onBack }: { onBack: () => void }) {
  ...
  if (!user) return <div className="min-h-screen bg-[radial-gradient(circle_at_70%_0%,rgba(36,108,255,.12),transparent_32%),linear-gradient(180deg,#020817,#041027)] text-white flex flex-col">
    <div className="w-full max-w-xl mx-auto px-5 pt-8 md:pt-10 text-center"><div className="inline-flex items-center gap-2 text-xs md:text-sm font-bold text-[#72a5ff]"><LockKeyhole className="w-4 h-4" /> CURSO DE APROVAÇÃO</div><h1 className="mt-4 text-3xl md:text-5xl font-extrabold tracking-[-0.045em] leading-[1.02]">Entre para abrir seu Curso.</h1><p className="mt-3 text-sm md:text-base text-[#a9bddc] leading-relaxed">Seu gêmeo de estudos, plano, faculdade-alvo, dificuldades e evolução ficam vinculados à sua conta.</p></div>
  ...
  const navItems=[['curso-inicio','Meu gêmeo',BrainCircuit],['curso-estrategia','Estratégia',Target],['curso-metas','Metas',BarChart3],['correcao-simulado','Simulados',ClipboardCheck],['curso-fases','Outras fases',Mic2],['curso-visual','Questões visuais',ScanLine],['curso-redacao','Redação',FileText]] as const;
  ...
  return <div className="relative min-h-screen bg-[#020817]">
    ...
    <div className="fixed bottom-[76px] left-3 z-[85] md:bottom-5 md:left-5"><button type="button" onClick={()=>jump('curso-inicio')} className="inline-flex items-center gap-2 rounded-xl border border-[#31588e] bg-[#0b2856]/95 px-3 py-3 text-xs font-extrabold text-white shadow-xl backdrop-blur"><BrainCircuit size={16}/>Atualizar meu gêmeo</button></div>
