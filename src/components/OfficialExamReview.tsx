...
export default function OfficialExamReview(){
  ...
  return <section className="max-w-[1180px] mx-auto px-4 md:px-6 py-8 text-white font-['Plus_Jakarta_Sans']">
    <div className="rounded-[20px] border border-[#241904] bg-[#0b0904] p-5 md:p-6">
      <div className="flex items-center gap-2 text-xs font-extrabold text-[#ffd45e]"><BookOpenCheck size={16}/>Correção de simulado oficial</div>
      <h2 className="mt-2 text-2xl md:text-3xl font-extrabold tracking-[-.04em]">Qual prova você fez?</h2>
      <p className="mt-2 text-sm text-[#e0aa18]">Escolha ano, aplicação e caderno/cor. Você pode localizar uma questão ou informar todas as que errou de uma vez.</p>
      <div className="mt-5 grid grid-cols-1 md:grid-cols-4 gap-3">
        <label className="text-xs text-[#e0aa18]">Ano<select className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white" value={year} onChange={e=>setYear(Number(e.target.value))}>{yearOptions.map(y=><option key={y}>{y}</option>)}</select></label>
        <label className="text-xs text-[#e0aa18]">Aplicação<select className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white" value={editionId} onChange={e=>setEditionId(e.target.value)}>{applications.map(e=><option key={e.id} value={e.id}>{e.application_label||e.application}</option>)}</select></label>
        <label className="text-xs text-[#e0aa18]">Caderno / cor<select className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white" value={bookletId} onChange={e=>{const id=e.target.value;setBookletId(id);const b=booklets.find(x=>x.id===id);if(b)setNumber(b.question_start??1);setResult(null);setBatchRows([]);setSkillGroups([]);setBatchMessage('')}}>{booklets.length?booklets.map(b=><option key={b.id} value={b.id}>Dia {b.day} · Caderno {b.booklet_code} · {b.color}</option>):<option value="">Caderno ainda em catalogação</option>}</select></label>
        <label className="text-xs text-[#e0aa18]">Questão individual<input className="mt-1 w-full rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-white" type="number" min={minQ} max={maxQ} value={number} onChange={e=>setNumber(Number(e.target.value))}/></label>
      </div>
      <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={findQuestion} disabled={!bookletId||loading} className="inline-flex items-center gap-2 rounded-xl bg-[#ffd45e] px-4 py-3 text-sm font-extrabold disabled:opacity-40"><Search size={16}/>{loading?'Buscando…':'Identificar questão'}</button>{selectedBooklet?.source_pdf_url&&<a className="inline-flex items-center gap-2 rounded-xl border border-[#241904] bg-[#0b0904] px-4 py-3 text-sm font-bold" href={selectedBooklet.source_pdf_url} target="_blank" rel="noreferrer">Abrir prova oficial <ExternalLink size={15}/></a>}{selectedBooklet?.answer_key_url&&<a className="inline-flex items-center gap-2 rounded-xl border border-[#241904] bg-[#0b0904] px-4 py-3 text-sm font-bold" href={selectedBooklet.answer_key_url} target="_blank" rel="noreferrer">Gabarito oficial <ExternalLink size={15}/></a>}</div>
      {result&&<div className="mt-5 rounded-2xl border border-[#241904] bg-[#0b0904] p-4"><div className="text-xs font-extrabold text-[#ffd45e]">Questão {result.question_number} · {result.official_exam_items?.area||'Área em classificação'}</div>{result.official_exam_items?.prompt_text&&<p className="mt-3 leading-relaxed">{result.official_exam_items.prompt_text}</p>}<div className="mt-3 text-sm"><b>Gabarito:</b> {result.answer_status==='annulled'?'Anulada':result.correct_option||'em validação'}</div>{result.official_exam_items?.skill_name&&<div className="mt-1 text-sm"><b>Habilidade:</b> {result.official_exam_items.skill_name}</div>}{result.official_exam_items?.explanation&&<div className="mt-3 rounded-xl bg-[#171105] p-3 text-sm text-[#ffd45e]"><b>Explicação:</b> {result.official_exam_items.explanation}</div>}</div>}

      <div className="mt-8 border-t border-[#241904] pt-6">
        <div className="flex items-center gap-2 text-xs font-extrabold text-[#ffd45e]"><ListChecks size={16}/>Erros em lote</div>
        <h3 className="mt-2 text-xl md:text-2xl font-extrabold tracking-[-.03em]">Cole todas as questões que você errou.</h3>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#e0aa18]">Exemplo: <b className="text-white">12, 37, 84, 102, 146</b>. Também aceita intervalos como <b className="text-white">90-95</b>. O Conectaê cruza cada número com a cor do seu caderno, agrupa os erros por habilidade e cria recuperação específica.</p>
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
          <textarea value={wrongRaw} onChange={e=>setWrongRaw(e.target.value)} placeholder="12, 37, 84, 102, 146" className="min-h-[92px] w-full resize-y rounded-xl border border-[#241904] bg-[#0b0904] p-3 text-sm text-white outline-none focus:border-[#ffd45e]"/>
          <button type="button" onClick={analyzeWrongBatch} disabled={!bookletId||!parsedWrong.length||batchLoading} className="inline-flex min-h-[52px] items-center justify-center gap-2 self-end rounded-xl bg-[#ffd45e] px-5 py-3 text-sm font-extrabold disabled:opacity-40"><Sparkles size={16}/>{batchLoading?'Analisando…':`Analisar ${parsedWrong.length||''} erros`}</button>
        </div>
        {parsedWrong.length>0&&<p className="mt-2 text-xs text-[#b88408]">Questões reconhecidas na entrada: {parsedWrong.join(', ')}</p>}
        {batchMessage&&<div className={`mt-4 flex gap-2 rounded-xl border p-3 text-sm ${batchRows.length?'border-[#241904] bg-[#171105] text-[#ffd45e]':'border-[#241904] bg-[#0b0904] text-[#ffd45e]'}`}>{batchRows.length?<CheckCircle2 size={17} className="shrink-0"/>:<AlertCircle size={17} className="shrink-0"/>}<span>{batchMessage}</span></div>}
        {missingNumbers.length>0&&<div className="mt-3 text-xs text-[#e0aa18]">Ainda não indexadas neste caderno: {missingNumbers.join(', ')}</div>}

        {skillGroups.length>0&&<div className="mt-6 space-y-4">
          <div><div className="text-xs font-extrabold uppercase tracking-[.08em] text-[#ffd45e]">Diagnóstico do simulado</div><h3 className="mt-1 text-xl font-extrabold">O que você precisa atacar primeiro</h3></div>
          {skillGroups.map((g,i)=><article key={g.key} className="rounded-2xl border border-[#241904] bg-[#0b0904] p-4 md:p-5">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="text-[11px] font-extrabold uppercase tracking-[.06em] text-[#ffd45e]">Prioridade {i+1} · {g.count} erro{g.count===1?'':'s'}</div><h4 className="mt-1 text-lg font-extrabold">{g.skill}</h4><p className="mt-1 text-xs text-[#e0aa18]">{g.subject} · {g.area} · questões {g.numbers.join(', ')}</p></div><a href={g.videoUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-[#6f4f08] bg-[#171105] px-3 py-2 text-xs font-extrabold"><Video size={15}/>Vídeos para esta habilidade</a></div>
            <div className="mt-4 rounded-xl bg-[#0b0904] p-3"><b className="text-sm">Plano de recuperação</b><p className="mt-1 text-xs leading-relaxed text-[#e0aa18]">Revisar o conceito-base de <b className="text-white">{g.skill}</b>, assistir a uma aula dirigida, resolver pelo menos {Math.max(12,g.count*6)} questões semelhantes e refazer as questões do simulado sem consultar o gabarito.</p></div>
            {g.practice.length>0?<div className="mt-4"><div className="text-xs font-extrabold text-[#ffd45e]">Questões semelhantes do banco</div><div className="mt-2 grid gap-2">{g.practice.map(p=><details key={p.id} className="rounded-xl border border-[#241904] bg-[#0b0904] p-3"><summary className="cursor-pointer text-sm font-bold">{p.area} · {p.skill_name}</summary><p className="mt-2 text-sm leading-relaxed text-[#fffaf0]">{p.prompt}</p>{p.explanation&&<p className="mt-2 text-xs leading-relaxed text-[#e0aa18]"><b>Correção:</b> {p.explanation}</p>}</details>)}</div></div>:<p className="mt-4 text-xs text-[#b88408]">O banco ainda está buscando questões autorais equivalentes para esta habilidade.</p>}
          </article>)}
          <div className="rounded-xl border border-[#241904] bg-[#0b0904] p-4 text-sm text-[#ffd45e]"><b>Plano atualizado:</b> essas dificuldades foram registradas como EXTRAS e entram na priorização do seu cronograma semanal.</div>
        </div>}
      </div>
    </div>
  </section>;
