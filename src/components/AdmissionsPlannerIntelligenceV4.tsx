...
export default function AdmissionsPlannerIntelligenceV4({ onBack }:{ onBack:()=>void }){
  ...
  return <div className="min-h-screen bg-[#050505] text-ink-50">
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#050505]/95 backdrop-blur-xl"><div className="max-w-[1450px] mx-auto px-5 md:px-8 py-4 flex items-center justify-between"><button onClick={onBack} className="inline-flex items-center gap-2 text-sm font-bold text-ink-300"><ArrowLeft className="w-4 h-4"/>Voltar</button><div className="flex items-center gap-2"><div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-300 to-amber-400 flex items-center justify-center"><BrainCircuit className="w-5 h-5 text-[#050505]"/></div><div><div className="font-black">Conectaê Intelligence</div><div className="text-[10px] uppercase tracking-[.14em] text-amber-200">Approval Engine v4</div></div></div><div className="hidden md:block text-xs text-amber-200">✓ salvo na sua conta</div></div></header>

    <main className="max-w-[1450px] mx-auto px-5 md:px-8 py-7 pb-24">
      <section className="grid xl:grid-cols-[1.15fr_.85fr] gap-5 mb-6">
        <div className="rounded-[28px] border border-white/10 bg-white/[0.035] p-6 md:p-8"><div className="inline-flex items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1.5 text-xs font-black text-amber-100"><Sparkles className="w-4 h-4"/>plano pessoal salvo automaticamente</div><h1 className="text-3xl md:text-5xl font-black tracking-[-.035em] leading-[1.02] mt-4">Estude o que realmente aumenta sua chance.</h1><p className="text-ink-300 mt-4 max-w-3xl">Curso, faculdade, prova, simulados, matérias fortes, horas disponíveis, questões e erros ficam conectados ao mesmo plano.</p><div className="grid sm:grid-cols-2 gap-3 mt-6"><label className="text-xs font-bold text-ink-400">Curso<select value={selectedArea} onChange={e=>setSelectedArea(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-[#0b0904] px-3 py-3 text-sm text-white">{areas.map(a=><option key={a.area_id} value={a.area_id}>{a.courses||a.name}</option>)}</select></label><label className="text-xs font-bold text-ink-400">Faculdade<select value={selectedUniversity} onChange={e=>setSelectedUniversity(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-[#0b0904] px-3 py-3 text-sm text-white">{filteredUniversities.map(u=><option key={u.area_university_id} value={u.area_university_id}>{u.university_name}</option>)}</select></label></div><div className="mt-3 text-xs text-amber-200">Curso e faculdade são salvos automaticamente na sua conta.</div></div>
        <div className="rounded-[28px] border border-amber-300/15 bg-gradient-to-br from-amber-300/[0.08] to-amber-400/[0.05] p-6"><div className="text-[10px] uppercase tracking-[.15em] text-ink-500 font-black">Prontidão estimada</div><div className="text-6xl font-black text-amber-200 mt-2">{readiness}%</div><div className="mt-4 h-2 rounded-full bg-white/5 overflow-hidden"><div className="h-full bg-amber-300 rounded-full" style={{width:`${readiness}%`}}/></div><div className="grid grid-cols-2 gap-3 mt-5"><div className="rounded-2xl bg-black/15 border border-white/10 p-4"><CalendarDays className="w-4 h-4 text-amber-200"/><div className="text-xs text-ink-500 mt-2">Até a prova</div><div className="font-black">{remainingDays==null?'por etapas':`${remainingDays} dias`}</div></div><div className="rounded-2xl bg-black/15 border border-white/10 p-4"><Target className="w-4 h-4 text-amber-200"/><div className="text-xs text-ink-500 mt-2">Meta global</div><div className="font-black">{target.target_value??'adaptativa'}{examId==='enem'?' pts':'%'}</div></div></div></div>
      </section>

      <nav className="flex gap-2 overflow-x-auto mb-6">{([['dashboard','Diagnóstico'],['plan','Plano semanal'],['practice','Questões'],['database','Banco da prova']] as [Tab,string][]).map(([id,label])=><button key={id} onClick={()=>setTab(id)} className={`whitespace-nowrap rounded-full border px-4 py-2.5 text-sm font-black ${tab===id?'border-amber-300/40 bg-amber-300/10 text-amber-100':'border-white/10 text-ink-400'}`}>{label}</button>)}</nav>
      {message&&<div className="mb-5 rounded-2xl border border-amber-300/20 bg-amber-300/[0.07] px-4 py-3 text-sm text-amber-100">{message}</div>}

      {tab==='dashboard'&&<div className="grid xl:grid-cols-[1fr_.85fr] gap-5">
        <section className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6"><div className="flex items-center justify-between gap-3"><div><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Seu nível atual</div><h2 className="text-2xl font-black mt-1">Simulado por componente</h2></div><button onClick={saveSimulation} disabled={saving} className="rounded-xl bg-amber-300 text-[#050505] px-4 py-2.5 font-black text-sm inline-flex items-center gap-2"><Save className="w-4 h-4"/>{saving?'Salvando...':'Salvar simulado'}</button></div><div className="space-y-5 mt-6">{metrics.map(m=>{const value=values[m.key]??0;const goal=targetValues[m.key]??m.max;return <div key={m.key}><div className="flex justify-between items-end mb-2"><div><div className="font-black">{m.label}</div><div className="text-xs text-amber-200">meta adaptativa: {goal} {m.unit} · {value>=goal?'meta atingida':`faltam ${Math.max(0,goal-value)}`}</div></div><div className="text-xl font-black">{value}</div></div><input type="range" min={0} max={m.max} step={m.max>100?10:1} value={value} onChange={e=>setValues(v=>({...v,[m.key]:Number(e.target.value)}))} className="w-full accent-amber-300"/></div>})}</div></section>
        <section className="space-y-5"><div className="rounded-[28px] border border-amber-300/25 bg-amber-300/[0.06] p-6"><div className="text-xs uppercase tracking-[.14em] text-amber-200 font-black">Matérias de conforto</div><h2 className="text-xl font-black mt-1">Onde você ganha pontos com mais facilidade?</h2><p className="text-xs text-ink-500 mt-2">Marcar uma força reduz a carga de recuperação, mas pode elevar a meta nessa área para compensar lacunas.</p><div className="flex flex-wrap gap-2 mt-4">{metrics.map(m=><button key={m.key} onClick={()=>toggleComfort(m.key)} className={`rounded-xl border px-3 py-2 text-sm font-bold ${comfortAreas.includes(m.key)?'border-amber-300/40 bg-amber-300/10 text-amber-100':'border-white/10 text-ink-400'}`}>{comfortAreas.includes(m.key)?'✓ ':''}{m.label}</button>)}</div><div className="mt-5"><label className="text-xs font-bold text-ink-400">Horas disponíveis por semana: <span className="text-white">{weeklyHours}h</span></label><input type="range" min={3} max={30} step={1} value={weeklyHours} onChange={e=>setWeeklyHours(Number(e.target.value))} className="w-full accent-amber-300 mt-2"/></div></div><div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6"><div className="text-xs uppercase tracking-[.14em] text-amber-200 font-black">Prioridades</div><div className="space-y-3 mt-4">{priorities.slice(0,5).map((p,i)=><div key={p.metric.key} className="rounded-2xl border border-white/10 bg-black/15 p-4 flex items-center justify-between gap-3"><div><div className="text-xs text-ink-500">#{i+1}{p.comfort?' · força declarada':''}</div><div className="font-black">{p.metric.label}</div><div className="text-xs text-ink-500">{p.current} → {p.goal} {p.metric.unit}</div></div><TrendingUp className="w-4 h-4 text-amber-200"/></div>)}</div></div></section>
      </div>}

      {tab==='plan'&&<section><div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-5"><div><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Plano de execução</div><h2 className="text-3xl font-black mt-1">{weeks} semanas até sua meta</h2><p className="text-sm text-ink-400 mt-2 max-w-3xl">Cada semana combina teoria, questões, correção de erros, revisão e simulado. As questões são selecionadas do banco da prova e podem ser respondidas aqui no Conectaê.</p></div><button onClick={()=>downloadPdf()} className="rounded-xl border border-amber-300/30 bg-amber-300/10 px-4 py-3 text-sm font-black text-amber-100 inline-flex items-center gap-2"><Download className="w-4 h-4"/>Plano completo + questões em PDF</button></div><div className="space-y-4">{weeklyPlan.map((w,idx)=><article key={w.week} className="rounded-[26px] border border-white/10 bg-white/[0.03] p-5 md:p-6"><div className="flex flex-col xl:flex-row xl:items-start justify-between gap-5"><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-amber-300/10 border border-amber-300/20 px-3 py-1 text-xs font-black text-amber-100">SEMANA {w.week}</span><span className="text-xs text-ink-500">{w.phase}</span></div><h3 className="text-2xl font-black mt-3">{w.focus} <span className="text-ink-600">+</span> {w.secondary}</h3><div className="grid sm:grid-cols-4 gap-2 mt-4">{[[BookOpen,`${w.theoryHours}h`,'Teoria'],[PlayCircle,`${w.practiceHours}h`,'Questões'],[TrendingUp,`${w.reviewHours}h`,'Correção'],[Flame,`${w.simulationHours}h`,'Simulado']].map(([Icon,h,label])=>{const I=Icon as typeof BookOpen;return <div key={String(label)} className="rounded-xl border border-white/10 bg-black/15 p-3"><I className="w-4 h-4 text-amber-200"/><div className="font-black mt-1">{String(h)}</div><div className="text-[10px] text-ink-500">{String(label)}</div></div>})}</div><div className="mt-4 rounded-2xl border border-amber-300/15 bg-amber-300/[0.04] p-4"><div className="text-[10px] uppercase tracking-wider text-amber-200 font-black">O que estudar</div><div className="flex flex-wrap gap-2 mt-2">{w.skills.map(s=><span key={s.skill_code} className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-ink-300">{s.skill_name}</span>)}</div></div><div className="mt-4 grid md:grid-cols-2 gap-3"><div className="rounded-2xl border border-white/10 p-4"><div className="text-xs font-black text-ink-300">Entregas da semana</div>{w.deliverables.map(d=><div key={d} className="flex gap-2 text-xs text-ink-500 mt-2"><CheckCircle2 className="w-3.5 h-3.5 text-amber-300 shrink-0"/>{d}</div>)}</div><div className="rounded-2xl border border-white/10 p-4"><div className="text-xs font-black text-ink-300">Checkpoint</div><div className="text-3xl font-black text-amber-200 mt-2">{w.checkpoint} {w.unit}</div><div className="text-xs text-ink-500 mt-1">meta intermediária para {w.focus}</div></div></div></div><aside className="xl:w-[360px] space-y-3"><div className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.05] p-4"><div className="font-black">{w.questions.length} questões selecionadas</div><div className="text-xs text-ink-500 mt-1">Com resposta e explicação dentro do site.</div><button onClick={()=>startPractice(w.questions)} className="mt-3 w-full rounded-xl bg-amber-300 text-[#050505] py-2.5 font-black text-sm">Resolver no Conectaê</button></div><div className="rounded-2xl border border-white/10 p-4"><div className="font-black text-sm flex items-center gap-2"><Video className="w-4 h-4 text-amber-200"/>Aulas e recursos</div><div className="space-y-2 mt-3">{w.resources.map(r=><a key={r.id} target="_blank" rel="noreferrer" href={r.url||youtubeSearch(`${r.search_query||r.title} ${w.skills[0]?.skill_name||w.focus}`)} className="block rounded-xl border border-white/10 bg-black/15 px-3 py-2.5 text-xs text-ink-300 hover:text-white">{r.official?'Fonte oficial: ':''}{r.title}<ExternalLink className="inline w-3 h-3 ml-1"/></a>)}</div></div><button onClick={()=>downloadPdf(idx)} className="w-full rounded-xl border border-white/10 py-3 text-sm font-black inline-flex items-center justify-center gap-2"><FileText className="w-4 h-4"/>PDF desta semana + gabarito</button></aside></div></article>)}</div></section>}

      {tab==='practice'&&<section className="max-w-4xl mx-auto"><div className="flex items-center justify-between gap-3 mb-5"><div><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Sala de questões</div><h2 className="text-3xl font-black mt-1">Resolva sem sair do Conectaê</h2></div>{practiceQueue.length>0&&<div className="text-sm text-ink-400">{practiceIndex+1}/{practiceQueue.length} · {sessionCorrect} acertos</div>}</div>{!activeQuestion?<div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-8 text-center"><Database className="w-10 h-10 text-ink-600 mx-auto"/><h3 className="font-black text-xl mt-3">Escolha uma lista</h3><p className="text-sm text-ink-500 mt-2">Você pode começar por uma semana do plano ou fazer uma lista geral da prova.</p><button onClick={()=>startPractice(examQuestions.slice(0,15))} className="mt-5 rounded-xl bg-amber-300 text-[#050505] px-5 py-3 font-black">Começar 15 questões de {profile?.label??LABELS[examId]}</button></div>:<div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6 md:p-8"><div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-amber-300/10 px-3 py-1 text-amber-100 font-black">{activeQuestion.area}</span><span className="rounded-full bg-white/5 px-3 py-1 text-ink-400">{activeQuestion.skill_name}</span><span className="rounded-full bg-white/5 px-3 py-1 text-ink-400">dificuldade {activeQuestion.difficulty}/5</span></div><div className="text-lg md:text-xl font-bold leading-relaxed mt-5">{activeQuestion.prompt}</div>{activeQuestion.correct_option?<div className="space-y-2 mt-6">{questionOptions(activeQuestion).map(([letter,opt])=><button key={letter} disabled={revealed} onClick={()=>setSelectedOption(letter)} className={`w-full text-left rounded-2xl border p-4 transition-all ${selectedOption===letter?'border-amber-300/50 bg-amber-300/10':'border-white/10 bg-black/15'} ${revealed&&letter===activeQuestion.correct_option?'!border-amber-300/50 !bg-amber-300/10':''}` }><span className="font-black mr-2">{letter})</span>{opt}</button>)}</div>:<textarea value={writtenAnswer} disabled={revealed} onChange={e=>setWrittenAnswer(e.target.value)} placeholder="Escreva sua resposta completa e justifique o raciocínio..." className="mt-6 w-full min-h-44 rounded-2xl border border-white/10 bg-[#0b0904] p-4 text-sm outline-none focus:border-amber-300/35"/>}{!revealed?<button onClick={answerPractice} disabled={activeQuestion.correct_option?!selectedOption:!writtenAnswer.trim()} className="mt-5 w-full rounded-xl bg-amber-300 text-[#050505] py-3.5 font-black disabled:opacity-40">Responder e ver correção</button>:<div className={`mt-5 rounded-2xl border p-5 ${practiceResult===false?'border-amber-300/20 bg-amber-300/[0.05]':'border-amber-300/20 bg-amber-300/[0.05]'}`}><div className="flex items-center gap-2 font-black">{practiceResult===false?<XCircle className="w-5 h-5 text-amber-300"/>:<CheckCircle2 className="w-5 h-5 text-amber-300"/>}{activeQuestion.correct_option?(practiceResult?'Correto':'Resposta incorreta'):'Resposta enviada — confira o critério'}</div>{activeQuestion.correct_option&&<div className="text-sm text-ink-300 mt-3">Gabarito: <strong>{activeQuestion.correct_option}</strong></div>}<div className="mt-3 text-sm leading-relaxed text-ink-300"><strong>Explicação:</strong> {activeQuestion.explanation||'Revise a habilidade e compare seu raciocínio com o conteúdo estudado.'}</div><button onClick={nextPractice} className="mt-4 rounded-xl bg-white text-[#050505] px-5 py-2.5 font-black text-sm">{practiceIndex===practiceQueue.length-1?'Concluir sessão':'Próxima questão'}</button></div>}</div>}</section>}

      {tab==='database'&&<div className="grid xl:grid-cols-2 gap-5"><section className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6"><div className="text-xs uppercase tracking-[.14em] text-amber-200 font-black">Banco da prova</div><h2 className="text-2xl font-black mt-1">{profile?.label??LABELS[examId]}</h2><p className="text-sm text-ink-400 mt-3 leading-relaxed">{profile?.format_summary||'Estrutura específica da seleção.'}</p><div className="grid grid-cols-3 gap-3 mt-5"><div className="rounded-xl border border-white/10 p-4"><div className="text-2xl font-black">{examQuestions.length}</div><div className="text-[10px] text-ink-500">QUESTÕES</div></div><div className="rounded-xl border border-white/10 p-4"><div className="text-2xl font-black">{examSkills.length}</div><div className="text-[10px] text-ink-500">HABILIDADES</div></div><div className="rounded-xl border border-white/10 p-4"><div className="text-2xl font-black">{examStudy.length}</div><div className="text-[10px] text-ink-500">RECURSOS</div></div></div><button onClick={()=>startPractice(examQuestions.slice(0,20))} className="mt-5 w-full rounded-xl bg-amber-300 text-[#050505] py-3 font-black">Fazer lista de 20 questões</button></section><section className="rounded-[28px] border border-white/10 bg-white/[0.03] p-6"><div className="text-xs uppercase tracking-[.14em] text-amber-200 font-black">Fontes e aulas</div><div className="space-y-3 mt-4">{examStudy.map(r=><a key={r.id} href={r.url||youtubeSearch(r.search_query||r.title)} target="_blank" rel="noreferrer" className="block rounded-2xl border border-white/10 bg-black/15 p-4"><div className="font-black text-sm">{r.title}</div><div className="text-xs text-ink-500 mt-1">{r.description}</div></a>)}{profile?.official_source_url&&<a href={profile.official_source_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-black text-amber-200 mt-3">Fonte oficial da prova <ExternalLink className="w-4 h-4"/></a>}</div></section></div>}
    </main>
  </div>;
}
