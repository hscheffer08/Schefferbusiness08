...
export default function AdmissionsPlannerIntelligence({ onBack }: { onBack: () => void }) {
  ...
  if (loading) return <div className="min-h-screen bg-[#050505] flex items-center justify-center"><Loader2 className="w-9 h-9 animate-spin text-amber-300" /></div>;

  return <div className="min-h-screen bg-[#050505] text-ink-50 relative overflow-hidden">
    <div className="fixed inset-0 pointer-events-none"><div className="absolute -top-48 -left-40 w-[620px] h-[620px] rounded-full bg-amber-500/12 blur-[145px]"/><div className="absolute top-1/4 -right-48 w-[620px] h-[620px] rounded-full bg-amber-500/10 blur-[150px]"/></div>
    <header className="relative z-10 border-b border-white/10 bg-[#050505]/90 backdrop-blur-xl sticky top-0"><div className="max-w-[1450px] mx-auto px-5 md:px-8 py-4 flex items-center justify-between gap-4"><button onClick={onBack} className="inline-flex items-center gap-2 text-sm font-bold text-ink-300 hover:text-white"><ArrowLeft className="w-4 h-4"/>Voltar</button><div className="flex items-center gap-2"><div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-300 to-amber-400 flex items-center justify-center"><BrainCircuit className="w-5 h-5 text-[#050505]"/></div><div><div className="font-black leading-none">Conectaê Intelligence</div><div className="text-[10px] tracking-[.15em] uppercase text-amber-200 mt-1">Approval Engine v3</div></div></div><div className="hidden md:flex items-center gap-2 text-xs text-ink-400"><ShieldCheck className="w-4 h-4 text-amber-300"/>plano adaptativo</div></div></header>
    <main className="relative z-10 max-w-[1450px] mx-auto px-5 md:px-8 py-7 pb-24">
      <section className="grid xl:grid-cols-[1.1fr_.9fr] gap-5 mb-6"><div className="rounded-[28px] border border-white/10 bg-white/[0.035] p-6 md:p-8"><div className="inline-flex items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1.5 text-xs font-black text-amber-200 mb-4"><Sparkles className="w-3.5 h-3.5"/>curso + prova + desempenho + rotina</div><h1 className="text-3xl md:text-5xl font-black tracking-[-.035em] leading-[1.02]">Seu caminho até a aprovação, recalculado em tempo real.</h1><p className="mt-4 text-ink-300 max-w-3xl leading-relaxed">Informe seu desempenho e suas matérias fortes. O Conectaê redistribui metas, horas, habilidades, questões e checkpoints conforme a prova e a faculdade.</p><div className="grid sm:grid-cols-2 gap-3 mt-6"><label className="text-xs font-bold text-ink-400">Curso<select value={selectedArea} onChange={e=>setSelectedArea(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-[#0b0904] px-3 py-3 text-sm text-white">{areas.map(a=><option key={a.area_id} value={a.area_id}>{a.courses||a.name}</option>)}</select></label><label className="text-xs font-bold text-ink-400">Faculdade<select value={selectedUniversity} onChange={e=>setSelectedUniversity(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-[#0b0904] px-3 py-3 text-sm text-white">{filteredUniversities.map(u=><option key={u.area_university_id} value={u.area_university_id}>{u.university_name}</option>)}</select></label></div></div><div className="rounded-[28px] border border-amber-300/15 bg-gradient-to-br from-amber-300/[0.08] to-amber-400/[0.05] p-6"><div className="text-[10px] uppercase tracking-[.15em] text-ink-500 font-black">Prontidão estimada</div><div className="text-6xl font-black text-amber-200 mt-2">{readiness}%</div><div className="mt-4 h-2 rounded-full bg-white/5 overflow-hidden"><div className="h-full bg-gradient-to-r from-amber-300 via-brand-300 to-amber-300" style={{width:`${readiness}%`}}/></div><div className="grid grid-cols-2 gap-3 mt-5"><div className="rounded-2xl border border-white/10 bg-black/15 p-4"><CalendarDays className="w-4 h-4 text-amber-200 mb-2"/><div className="text-xs text-ink-500">Até a prova</div><div className="font-black mt-1">{remainingDays===null?'por etapas':`${remainingDays} dias`}</div></div><div className="rounded-2xl border border-white/10 bg-black/15 p-4"><Target className="w-4 h-4 text-amber-200 mb-2"/><div className="text-xs text-ink-500">Meta global</div><div className="font-black mt-1">{target.target_value ?? 'adaptativa'}{examId==='enem'?' pts':'%'}</div></div></div><button onClick={()=>downloadPdf()} className="mt-3 w-full rounded-xl border border-white/10 bg-white/5 py-3 text-sm font-black inline-flex items-center justify-center gap-2"><Download className="w-4 h-4"/>Baixar plano completo em PDF</button></div></section>

      <section className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-6">{EXAM_ORDER.map(id=>{const p=profiles.find(x=>x.exam_id===id);return <button key={id} onClick={()=>setExamId(id)} className={`rounded-2xl border p-4 text-left ${id===examId?'border-amber-300/45 bg-amber-300/10':'border-white/10 bg-white/[.025]'}`}><GraduationCap className="w-4 h-4 text-amber-200"/><div className="font-black text-sm mt-3">{p?.label??FALLBACK_LABELS[id]}</div><div className="text-[11px] text-ink-500 mt-1">{p?.exam_date?new Date(`${p.exam_date}T12:00:00`).toLocaleDateString('pt-BR'):'por etapas'}</div></button>})}</section>
      <nav className="flex gap-2 overflow-x-auto mb-6 pb-1">{([['dashboard','Diagnóstico'],['plan','Plano semanal'],['scanner','Scanner de questões'],['database','Banco da prova']] as [Tab,string][]).map(([id,label])=><button key={id} onClick={()=>setTab(id)} className={`whitespace-nowrap rounded-full px-4 py-2.5 text-sm font-black border ${tab===id?'border-amber-300/40 bg-amber-300/10 text-amber-100':'border-white/10 bg-white/[.025] text-ink-400'}`}>{label}</button>)}</nav>
      {message&&<div className="mb-5 rounded-2xl border border-amber-300/20 bg-amber-300/[.06] px-4 py-3 text-sm text-amber-100">{message}</div>}
      ...
      {tab==='dashboard'&&<div className="space-y-5"><section className="rounded-[28px] border border-amber-300/25 bg-amber-300/[.055] p-6"><div className="flex flex-col md:flex-row md:items-center justify-between gap-4"><div><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Sua estratégia pessoal</div><h2 className="text-2xl font-black mt-1">Quais matérias são mais confortáveis para você?</h2><p className="text-sm text-ink-400 mt-2">Marque suas forças. O motor pode exigir mais delas e aliviar áreas fracas sem abandonar a meta global.</p></div><div className="min-w-52"><div className="text-xs font-bold text-ink-400 mb-2">Horas disponíveis por semana</div><input type="range" min={2} max={30} value={weeklyHours} onChange={e=>setWeeklyHours(Number(e.target.value))} className="w-full accent-amber-300"/><div className="font-black text-amber-100 mt-1">{weeklyHours} horas/semana</div></div></div><div className="flex flex-wrap gap-2 mt-5">{metrics.map(m=><button key={m.key} onClick={()=>toggleComfort(m.key)} className={`rounded-xl border px-4 py-3 text-sm font-black ${comfortAreas.includes(m.key)?'border-amber-300/50 bg-amber-300/15 text-amber-100':'border-white/10 bg-black/15 text-ink-400'}`}>{m.label}{comfortAreas.includes(m.key)?' ✓ força':''}</button>)}</div><button onClick={savePreferences} disabled={saving} className="mt-4 rounded-xl border border-amber-300/25 px-4 py-2.5 text-xs font-black text-amber-100">Salvar estratégia</button></section>
        <div className="grid xl:grid-cols-[1fr_.8fr] gap-5"><section className="rounded-[28px] border border-white/10 bg-white/[.03] p-6"><div className="flex items-center justify-between gap-4 mb-5"><div><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Seu nível agora</div><h2 className="text-2xl font-black mt-1">Desempenho por componente</h2></div><button onClick={saveAttempt} disabled={saving} className="rounded-xl bg-amber-300 text-[#050505] px-4 py-2.5 font-black text-sm inline-flex items-center gap-2"><Save className="w-4 h-4"/>Registrar simulado</button></div><div className="space-y-5">{metrics.map(m=>{const value=values[m.key]??0;const goal=targetValues[m.key]??m.max;return <div key={m.key}><div className="flex justify-between gap-3 mb-2"><div><div className="font-bold">{m.label}</div><div className="text-xs text-amber-200">meta adaptativa agora: <b>{goal} {m.unit}</b></div><div className="text-[11px] text-ink-600 mt-0.5">faltam {Math.max(0,goal-value)} {m.unit}</div></div><div className={`text-xl font-black ${value>=goal?'text-amber-300':'text-white'}`}>{value}</div></div><input type="range" min={0} max={m.max} step={m.max>100?10:1} value={value} onChange={e=>setValues(v=>({...v,[m.key]:Number(e.target.value)}))} className="w-full accent-amber-300"/></div>})}</div>{examId==='enem'&&<div className="mt-5 rounded-2xl border border-amber-300/15 bg-amber-300/[.05] p-4 text-xs text-amber-100/80">Acertos no ENEM são usados como meta de consistência. A TRI não permite converter um número fixo de acertos em nota exata.</div>}</section><section className="rounded-[28px] border border-white/10 bg-white/[.03] p-6"><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Motor adaptativo</div><h2 className="text-2xl font-black mt-1 mb-5">Onde cada hora vale mais</h2><div className="space-y-3">{priorities.map((p,i)=><div key={p.metric.key} className="rounded-2xl border border-white/10 bg-black/15 p-4"><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><span className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center font-black">{i+1}</span><div><div className="font-black">{p.metric.label}{p.comfort&&<span className="ml-2 text-[10px] text-amber-200">FORÇA</span>}</div><div className="text-xs text-ink-500 mt-1">{p.current} → {p.goal} {p.metric.unit}</div></div></div><TrendingUp className="w-4 h-4 text-amber-200"/></div>{p.skills.length>0&&<div className="flex flex-wrap gap-1.5 mt-3">{p.skills.map(s=><span key={s.skill_code} className="rounded-full bg-white/5 border border-white/10 px-2.5 py-1 text-[10px]">{s.skill_name}</span>)}</div>}</div>)}</div></section></div></div>}

      {tab==='plan'&&<section className="space-y-4"><div className="rounded-[28px] border border-white/10 bg-white/[.03] p-6 flex flex-col md:flex-row md:items-center justify-between gap-4"><div><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">{weeks} semanas · {weeklyHours}h/semana</div><h2 className="text-3xl font-black mt-1">Plano executável, não só metas</h2><p className="text-sm text-ink-400 mt-2">Cada semana combina checkpoint, horas por prioridade, habilidades, prática e conteúdo.</p></div><div className="text-right"><div className="text-xs text-ink-500">Prática nesta sessão</div><div className="font-black text-xl">{practiceCorrect}/{practiceTotal} corretas</div></div></div><div className="grid lg:grid-cols-2 gap-4">{weeklyPlan.map((w,idx)=><article key={w.week} className="rounded-[26px] border border-white/10 bg-white/[.025] p-5"><div className="flex items-center justify-between"><div><span className="text-xs font-black text-amber-200">SEMANA {w.week}</span><h3 className="text-xl font-black mt-1">{w.focus}</h3></div><Flame className="w-5 h-5 text-amber-200"/></div><div className="text-sm text-ink-400 mt-1">{w.phase}</div><div className="grid grid-cols-3 gap-2 mt-4"><div className="rounded-xl bg-black/15 border border-white/10 p-3"><div className="text-[10px] text-ink-500 uppercase">checkpoint</div><div className="font-black mt-1">{w.checkpoint} {w.unit}</div></div><div className="rounded-xl bg-black/15 border border-white/10 p-3"><div className="text-[10px] text-ink-500 uppercase">prioridade 1</div><div className="font-black mt-1">{w.hrs1}h</div><div className="text-[10px] text-ink-500">{w.p1?.metric.label}</div></div><div className="rounded-xl bg-black/15 border border-white/10 p-3"><div className="text-[10px] text-ink-500 uppercase">prioridade 2</div><div className="font-black mt-1">{w.hrs2}h</div><div className="text-[10px] text-ink-500">{w.p2?.metric.label}</div></div></div>{w.p1?.skills.length? <div className="mt-4"><div className="text-[10px] uppercase tracking-wider text-ink-500 mb-2">habilidades da semana</div><div className="flex flex-wrap gap-1.5">{w.p1.skills.map(s=><span key={s.skill_code} className="rounded-full border border-amber-300/15 bg-amber-300/[.06] px-2.5 py-1 text-[10px] text-amber-100">{s.skill_name}</span>)}</div></div>:null}<div className="grid sm:grid-cols-3 gap-2 mt-4"><button onClick={()=>openQuestion(w.q[0])} disabled={!w.q.length} className="rounded-xl bg-amber-300 text-[#050505] py-2.5 text-xs font-black inline-flex items-center justify-center gap-1.5 disabled:opacity-35"><PlayCircle className="w-4 h-4"/>Fazer questões</button>{w.sr.find(r=>r.resource_type==='video_search')?<a target="_blank" rel="noreferrer" href={`https://www.youtube.com/results?search_query=${encodeURIComponent(`${w.sr.find(r=>r.resource_type==='video_search')?.search_query ?? profile?.label ?? ''} ${w.focus}`)}`} className="rounded-xl border border-amber-300/20 bg-amber-300/[.06] py-2.5 text-xs font-black text-amber-100 inline-flex items-center justify-center gap-1.5"><Video className="w-4 h-4"/>Ver aula</a>:<button disabled className="rounded-xl border border-white/10 py-2.5 text-xs text-ink-600">Vídeo indisponível</button>}<button onClick={()=>downloadPdf(idx)} className="rounded-xl border border-white/10 bg-white/5 py-2.5 text-xs font-black inline-flex items-center justify-center gap-1.5"><Download className="w-4 h-4"/>PDF da semana</button></div>{w.sr.filter(r=>r.url).slice(0,2).map(r=><a key={r.id} href={r.url!} target="_blank" rel="noreferrer" className="mt-2 block text-xs text-amber-200 hover:underline">{r.official?'Fonte oficial: ':''}{r.title} ↗</a>)}</article>)}</div></section>}

      {tab==='scanner'&&<div className="grid xl:grid-cols-[1fr_.85fr] gap-5"><section className="rounded-[28px] border border-white/10 bg-white/[.03] p-6"><div className="font-black text-xl mb-4">Scanner de questão</div><label className="block rounded-2xl border border-dashed border-amber-300/25 bg-amber-300/[.04] p-5 text-center cursor-pointer"><Camera className="w-6 h-6 text-amber-200 mx-auto mb-2"/><div className="font-bold text-sm">Enviar foto ou print</div><input type="file" accept="image/*" className="hidden" onChange={e=>void handleImage(e.target.files?.[0]??null)}/></label><textarea value={questionText} onChange={e=>setQuestionText(e.target.value)} placeholder="Cole aqui o enunciado completo..." className="mt-4 w-full min-h-44 rounded-2xl border border-white/10 bg-[#0b0904] p-4 text-sm"/><button onClick={()=>classify(questionText)} disabled={!questionText.trim()} className="mt-3 w-full rounded-xl bg-amber-300 text-[#050505] py-3 font-black disabled:opacity-40">Diagnosticar habilidade</button></section><section className="rounded-[28px] border border-white/10 bg-white/[.03] p-6">{scannerResult?<><div className="text-xs font-black text-amber-200">{scannerConfidence}% confiança</div><h3 className="text-2xl font-black mt-3">{scannerResult.skill_name}</h3><div className="text-sm text-ink-400">{scannerResult.area}</div><div className="mt-5"><div className="text-xs font-black">Se errou, qual foi o motivo?</div><div className="flex flex-wrap gap-2 mt-3">{['conteúdo','interpretação','procedimento','tempo','desatenção'].map(t=><button key={t} onClick={()=>setErrorType(t)} className={`rounded-full px-3 py-1.5 text-xs font-bold border ${errorType===t?'border-amber-300/40 bg-amber-300/10':'border-white/10'}`}>{t}</button>)}</div></div><div className="grid grid-cols-2 gap-2 mt-4"><button onClick={()=>void saveDiagnostic(false)} className="rounded-xl border border-amber-300/20 bg-amber-300/[.07] py-3 text-sm font-black">Errei</button><button onClick={()=>void saveDiagnostic(true)} className="rounded-xl border border-amber-300/20 bg-amber-300/[.07] py-3 text-sm font-black">Acertei</button></div></>:<div className="min-h-72 flex flex-col items-center justify-center text-center text-ink-500"><FileScan className="w-10 h-10 mb-3"/>Envie uma questão para conectá-la ao mapa de habilidades.</div>}</section></div>}

      {tab==='database'&&<div className="grid xl:grid-cols-[.9fr_1.1fr] gap-5"><section className="rounded-[28px] border border-white/10 bg-white/[.03] p-6"><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Perfil da prova</div><h2 className="text-2xl font-black mt-1">{profile?.label??FALLBACK_LABELS[examId]}</h2><p className="mt-4 text-sm text-ink-300 leading-relaxed">{profile?.format_summary}</p><div className="grid grid-cols-3 gap-2 mt-5"><div className="rounded-xl bg-black/15 border border-white/10 p-3"><BookOpen className="w-4 h-4 text-amber-200"/><div className="font-black mt-2">{examSkills.length}</div><div className="text-[10px] text-ink-500">habilidades</div></div><div className="rounded-xl bg-black/15 border border-white/10 p-3"><Database className="w-4 h-4 text-amber-200"/><div className="font-black mt-2">{examQuestions.length}</div><div className="text-[10px] text-ink-500">questões internas</div></div><div className="rounded-xl bg-black/15 border border-white/10 p-3"><Video className="w-4 h-4 text-amber-200"/><div className="font-black mt-2">{examStudyResources.length}</div><div className="text-[10px] text-ink-500">recursos</div></div></div>{profile?.official_source_url&&<a href={profile.official_source_url} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-black text-amber-200">Fonte oficial <ExternalLink className="w-4 h-4"/></a>}</section><section className="rounded-[28px] border border-white/10 bg-white/[.03] p-6"><div className="text-xs uppercase tracking-[.14em] font-black text-amber-200">Fontes e materiais</div><div className="space-y-3 mt-4">{[...examStudyResources,...examResources.map(r=>({id:100000+r.id,exam_id:r.exam_id,area:null,skill_name:null,resource_type:r.resource_type,title:r.label,url:r.url,search_query:null,description:null,official:true,priority:100} as StudyResource))].slice(0,12).map(r=><div key={r.id} className="rounded-2xl border border-white/10 bg-black/15 p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-black text-sm">{r.title}</div><div className="text-xs text-ink-500 mt-1">{r.official?'oficial · ':''}{r.resource_type.replaceAll('_',' ')}</div></div>{r.url?<a href={r.url} target="_blank" rel="noreferrer"><ExternalLink className="w-4 h-4 text-amber-200"/></a>:r.search_query?<a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(r.search_query)}`} target="_blank" rel="noreferrer"><Video className="w-4 h-4 text-amber-200"/></a>:null}</div>{r.description&&<p className="text-xs text-ink-500 mt-2">{r.description}</p>}</div>)}</div></section></div>}

      {activeQuestion&&<div className="fixed inset-0 z-[120] bg-black/70 backdrop-blur-sm p-4 overflow-y-auto"><div className="max-w-2xl mx-auto my-8 rounded-[28px] border border-white/10 bg-[#0b0904] p-6"><div className="flex justify-between gap-4"><div><div className="text-xs uppercase tracking-wider text-amber-200 font-black">{activeQuestion.area} · dificuldade {activeQuestion.difficulty}/5</div><div className="text-xs text-ink-500 mt-1">{activeQuestion.skill_name}</div></div><button onClick={()=>setActiveQuestion(null)} className="text-ink-400"><XCircle className="w-6 h-6"/></button></div><h3 className="text-xl font-black mt-5 leading-relaxed">{activeQuestion.prompt}</h3>{activeQuestion.correct_option?<div className="space-y-2 mt-5">{(['A','B','C','D','E'] as const).map(letter=>{const text=activeQuestion[`option_${letter.toLowerCase()}` as keyof PracticeQuestion] as string|null;if(!text)return null;return <button key={letter} onClick={()=>practiceResult===null&&setSelectedOption(letter)} className={`w-full text-left rounded-xl border p-3 text-sm ${selectedOption===letter?'border-amber-300/50 bg-amber-300/10':'border-white/10 bg-black/15'}`}><b>{letter})</b> {text}</button>})}</div>:<textarea className="mt-5 w-full min-h-36 rounded-xl border border-white/10 bg-black/20 p-4 text-sm" placeholder="Escreva sua resposta discursiva aqui..."/>}<button onClick={()=>void checkPractice()} disabled={!!activeQuestion.correct_option&&!selectedOption} className="mt-5 w-full rounded-xl bg-amber-300 text-[#050505] py-3 font-black disabled:opacity-40">Corrigir</button>{practiceResult!==null&&<div className={`mt-4 rounded-xl border p-4 ${practiceResult?'border-amber-300/25 bg-amber-300/[.07]':'border-amber-300/25 bg-amber-300/[.07]'}`}><div className="font-black">{practiceResult?'Correto':'Ainda não'}</div><div className="text-sm text-ink-300 mt-2">{activeQuestion.explanation}</div></div>}{!activeQuestion.correct_option&&message&&<div className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[.06] p-4 text-sm text-amber-100">{activeQuestion.explanation||'Compare sua resposta com os elementos essenciais esperados.'}</div>}<div className="grid grid-cols-2 gap-2 mt-4"><button onClick={()=>openQuestion()} className="rounded-xl border border-white/10 py-2.5 text-sm font-black">Próxima questão</button><button onClick={()=>setActiveQuestion(null)} className="rounded-xl border border-white/10 py-2.5 text-sm text-ink-400">Voltar ao plano</button></div></div></div>}

      <footer className="mt-8 grid md:grid-cols-4 gap-3">{[[Database,'Banco por prova',`${examSkills.length} habilidades`],[BarChart3,'Meta adaptativa','muda com seu desempenho'],[Clock3,'Rotina real',`${weeklyHours}h por semana`],[PlayCircle,'Prática ativa',`${examQuestions.length} questões disponíveis`]].map(([Icon,title,text])=>{const I=Icon as typeof Database;return <div key={String(title)} className="rounded-2xl border border-white/10 bg-white/[.025] p-4"><I className="w-4 h-4 text-amber-200 mb-3"/><div className="font-black text-sm">{String(title)}</div><div className="text-xs text-ink-500 mt-1">{String(text)}</div></div>})}</footer>
    </main>
  </div>;
}
